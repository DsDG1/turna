...
/// Compact new-vs-existing collision summary.
class CollisionStrip extends StatelessWidget {
  final int newCount;
...
/// One strategy radio row. The "consequence" line tells the user the data
/// impact of choosing this option; force-replace is the only destructive
...
/// editor.
class ReasonDisclosure extends StatefulWidget {
  const ReasonDisclosure({super.key, required this.reason});

  final String reason;

  @override
  State<ReasonDisclosure> createState() => _ReasonDisclosureState();
}

class _ReasonDisclosureState extends State<ReasonDisclosure> {
  var _expanded = false;
...
/// Human-readable label for the automatic layout classification.
String mappingTypeLabel(NotetypeMappingType type) {
...
/// Strategy label / description / consequence helpers.
String strategyLabel(BuildContext context, ImportStrategy s) {
  return switch (s) {
...
String strategyDescription(BuildContext context, ImportStrategy s) {
  return switch (s) {
...
String strategyConsequence(ImportStrategy s) {
  return switch (s) {
