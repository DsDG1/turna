...
class ReleaseManifest {
  ...
  static const List<JourneyStep> journeySteps = [
    ...
    JourneyStep(
      label: '0.8',
      title: '导入重做与复习升级',
      subtitle: '导入先试后定、删除真回收、跨牌组复习与朗读总开关',
    ),
  ];
  ...
  static const List<ChangelogRelease> fallbackReleases = [
    ChangelogRelease(
      version: '0.8',
      title: 'Anki 导入重做与复习升级',
      items: [
        'Anki 导入流程全面重做：牌组先完整试装、预览确认后才真正加入课程，中途取消或出错不会留下残余数据',
        '修复导入完成瞬间可能卡死或闪退的问题；几万张卡的大牌组导入时界面依然流畅',
        '导入时的字段匹配与课程摆放选择会随牌组数据一起保存，备份和恢复时一并带走',
        '删除牌组真正「删干净」：卡片、课程条目和占用的空间一起回收',
        '上一次导入还没结束时不再允许发起新导入，避免出现重复卡片',
        '导入预览页面焕新，确认字段匹配更直观',
        '「复习全部」可在多个牌组之间连续进行，中断后也能接着刷',
        '记忆保持曲线更准确，更容易看出哪些内容快忘了',
        '课程分节信息改用底部弹窗展示，浏览更省心',
        '新增朗读总开关，可一键关闭所有自动朗读',
        '学习页滚动更顺滑、界面动效更统一；内部清理两万多行旧代码，应用更小、更稳',
      ],
    ),
    ChangelogRelease(
      version: '0.7.2',
