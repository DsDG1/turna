...
- [主界面与功能使用指南](#主界面与功能使用指南)
  - [1. 导航与视图 (Activity Bar 与快捷键)](#1-导航与视图-activity-bar-与快捷键)
...
界面为**单壳多视图**布局（2026-09 焕新）：左侧 48px Activity Bar 导航轨，中央按视图切换的 QStackedWidget（欢迎 / 编辑 / 总览 / 资源），右侧 Copilot Dock，底部状态栏上方悬浮 AI 待应用动作条（PreviewHost strip）。

```
+--+----------+----------------------------------------------+-----------+
|≡ |          |  View Header（面包屑 + 节点徽标 + 动作）      | Copilot  |
|--|  侧栏     |                                              |  Dock     |
|📁|  (搜索    |  ┌────────────────────────────────────────┐  | (Ctrl+J)  |
|✨|  +课程树  |  │ 编辑视图: 详情面板 [编辑|预览|JSON]      │  |           |
|🗺|  +工具条) │  │ 总览视图: 整页鸟瞰 + 校验摘要            │  │ Experience│
|📚|          │  │ 资源视图: 本地资源 | Git 资源库          │  │  Dock     │
|🤖|          │  └────────────────────────────────────────┘  │           |
|--|          |     （无仓库时整屏替换为欢迎视图）            |           |
|🎓|          |                                              |           |
|⚙ |          |                                              |           |
+--+----------+----------------------------------------------+-----------+
|  [PreviewHost strip: AI 待应用动作 Enter 应用 / Esc 丢弃（有 pending 才出现）]  |
+-----------------------------------------------------------------------------------+
| 状态栏: JobTray │ ●未保存/✓已保存 14:32 │ 健康 │ 密度 ▾ │ 主题 ▾                  |
+-----------------------------------------------------------------------------------+
```

Activity Bar 角标：总览 = 校验错误数（红）、工坊 = 未完成导入的项目数、Copilot = 未读建议数（Dock 可见即已读）。

### 1. 导航与视图 (Activity Bar 与快捷键)

全局工具栏已取消，动作收编进 Activity Bar 与各视图头部：

| 入口 | 功能说明 | 快捷键 / 备注 |
|---|---|---|
| **应用菜单（≡）** | 打开/新建课程目录、最近仓库、发布、生成听力音频、命令面板、设置 | 顶部菜单按钮 |
| **编辑视图（📁）** | 侧栏课程树 + 详情面板（编辑/预览/JSON 三 tab） | `Ctrl+1` |
| **工坊（✨）** | 启动一站式 AI 协同创作窗口（从教材提取、AI 设计、审校到幂等导入） | `Ctrl+2`，非模态窗口，可并行操作 |
| **总览（🗺）** | 全局课程结构鸟瞰图：多行统计（Sections/Units/Lessons/空课时/校验错警 + 词汇·表达·语法覆盖率 + 课型分布）、每 Section 题型构成行、按名称/ID 搜索、课型徽标过滤、空课时灰边标记、一键导出 Markdown、点校验数跳转已有校验面板 | `Ctrl+3`；角标红色显示校验错误数 |
| **资源（📚）** | 开启本地词汇 (`vocab`)、表达 (`expressions`) 与语法点 (`grammar_points`) 编辑表格；页内第二 tab 为 Git 资源库 | `Ctrl+4`；支持 CSV 一键导出/导入 |
| **Copilot（🤖）** | 右侧 AI 建议面板（ExperienceDock），建议的待应用动作落在状态栏上方确认条 | `Ctrl+J` toggle |
| **教师模式（🎓）** | 切换为教师审查视角，提供课时结构化呈现与实时交互试做体验 | Activity Bar 底部 |
| **设置（⚙）** | 配置全局主题、密度、字体缩放、AI 接口服务（Base URL / Model / Key）、撤销步数与用量监控 | `Ctrl+,` |
| **命令面板** | ⌘K 式命令派发：转到视图 / 执行动作 / 定位课程节点 | `Ctrl+K` |
| **侧栏开关** | 编辑视图侧栏（搜索 + 课程树）显隐 | `Ctrl+B` |
| **保存** | 将当前修改写盘，自动触发 `course_cli validate` 和 `lint` 校验；校验失败自动回滚 | `Ctrl+S` |
| **发布** | 执行版本 Bump、音效 Manifest 挂载检查、Diff 差异生成与报告导出 | 应用菜单内 |

布局持久化：上次视图、窗口状态、侧栏/Copilot 显隐均经 QSettings 重启还原。

### 2. 课程树状导航 (Course Tree)

侧栏（编辑视图独有，`Ctrl+B` 收起）顶部搜索框支持按名称/ID/课型过滤（自动保留匹配节点的祖先链），树节点带 Lucide 图标与模板色徽标，拖拽时有 accent 色插入指示线。树展示 `Section` -> `Unit` -> `Lesson` 的三级包含关系：
- **右键菜单**：
...
tool/gui/
├── src/
│   ├── main.py                   # 程序主入口（--gallery 打开组件画廊）
│   ├── app.py                    # MainWindow（宿主属性 + 委托方法）
│   ├── theme.py                  # 主题应用（调色板 + 密度 + UI 缩放）
│   ├── theme_tokens.py           # 设计 token 单一真源（色/间距/圆角/字阶/层级/密度，Qt-free）
│   ├── icons/                    # vendored Lucide SVG ×52 + DPR 渲染/着色/缓存
│   ├── theme_styles/             # 模块化 QSS（core/buttons/inputs/views/shell，builder 组装）
│   ├── application/              # 应用层逻辑
│   │   ├── shell_views.py        # W2 壳层：ActivityBar/CentralStack/欢迎视图/快捷键/持久化/角标管线
│   │   ├── main_window_shell.py  # 工具栏 QAction、状态栏（保存态/密度/主题）装配
...
│   └── widgets/                  # 可复用 UI 组件
│       ├── ui/                   # UI Kit：ActivityBar/Badge/Card/EmptyState/Pill/SearchField/Toast/TurnaButton/TurnaDialog/ViewHeader + gallery
│       ├── tree_sidebar.py       # W3 编辑视图侧栏（搜索过滤 + 迷你工具条）
│       ├── welcome_view.py       # W2 欢迎视图（新建/打开/最近仓库）
│       ├── unified_workspace.py  # 创意画布三栏布局
│       ├── ai_orbit.py           # 中栏 AI 轨道
│       ├── course_tree.py        # 3 级课程结构树（Lucide 节点图标 + 过滤 + 拖拽指示线）
│       ├── detail_panel.py       # ViewHeader + 编辑|预览|JSON 三 tab 编辑器容器
