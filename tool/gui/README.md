...
自 P0 起，信息架构为 **项目库 ↔ 创意画布**（不再使用可点击的六阶段侧栏）。头栏 checklist 提示进度：`○/✓ 素材 · 知识 · 草稿 · 导入`。
...
| │ 知识点审校       │  │ 主题 / 等级 / 模板      │  │ 设计与草稿：聊天 · 模板 · JSON（高级）       │ |
...
4. **对话入口**：「打开对话 / 附件 / 模板…」切到右栏「设计与草稿」（许愿聊天、附件、Prompt 库、`[genre]`）。
...
| **设计与草稿** | 完整 DesignPanel：参数、聊天、模板、JSON（导入以编辑器内容为真相源）、通俗解释 |
