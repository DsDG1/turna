...
def _ctx_with_image_attachment():
    from types import SimpleNamespace
    ...
    adapter = SimpleNamespace(
        ...
        index={"language": "tr"}, course_dir=None,
    )
    img = AttachmentRecord(
        temp_path=Path("/tmp/x.png"),
        original_name="x.png",
    ...
    snaps = build_attachment_snapshot([img, txt])
    return build_experience_context(
        adapter, include_quality=False, include_hygiene=False, attachments=snaps
    )


class DockOcrSuggestionTest(unittest.TestCase):
    def test_suggestion_when_enabled_and_image_attachment(self):
        from src.backend.experience import local_suggestions

        ctx = _ctx_with_image_attachment()
        sugs = local_suggestions(ctx, limit=5, ocr_enabled=True)
        hit = [s for s in sugs if s["action_id"] == ACTION_ID]
        self.assertEqual(len(hit), 1)
        self.assertEqual(hit[0]["priority"], 2)
        self.assertIn("OCR 1 个图片附件", hit[0]["title"])
        # scope closed-set: count only, no ref_id/原文/path.
        self.assertEqual(hit[0]["scope"], {"count": 1})

    def test_no_suggestion_when_disabled(self):
        from src.backend.experience import local_suggestions

        ctx = _ctx_with_image_attachment()
        sugs = local_suggestions(ctx, limit=5, ocr_enabled=False)
        self.assertFalse(any(s["action_id"] == ACTION_ID for s in sugs))

    def test_no_suggestion_without_image_attachment(self):
        from types import SimpleNamespace
        ...
        snaps = build_attachment_snapshot([txt])
    ...
    def test_scope_carries_no_secret_text(self):
        from src.backend.experience import local_suggestions

        ctx = _ctx_with_image_attachment()
        sugs = local_suggestions(ctx, limit=5, ocr_enabled=True)
        hit = [s for s in sugs if s["action_id"] == ACTION_ID][0]
        blob = str(hit["scope"]) + hit["title"]
        self.assertNotIn("/tmp", blob)
        self.assertNotIn("base64", blob)


class ShellOcrEnabledHintTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        qt_app()
    ...
    def _make_shell(self):
        ...
        adapter = SimpleNamespace(
            sections=[], vocab=[], expressions=[], grammar_points=[],
            index={"language": "tr"}, course_dir="/tmp/x",
        )
    ...
    def test_set_ocr_enabled_drives_suggestion(self):
        ...
        shell.set_attachments(build_attachment_snapshot([img]))
        shell.set_ocr_enabled(True)
        ctx = shell.rebuild_now()
        self.assertTrue(
            any(s["action_id"] == ACTION_ID for s in shell._suggestions)
        )
        shell.set_ocr_enabled(False)
        shell.rebuild_now()
        self.assertFalse(
            any(s["action_id"] == ACTION_ID for s in shell._suggestions)
        )
