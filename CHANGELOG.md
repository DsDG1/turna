...
## 0.8 Anki 导入重做与复习升级 (2026-09-04)

本版是 Anki 底座的一次大版本收口：导入链整体换新、删除真正回收、复习跨牌组续接，并完成两轮大规�模内部清理（Step 6 与 Step 6.5，合计净删约两万行旧代码）。

### 改进

- **导入改为「先试装、后确认」**：牌组先在独立试用库完整导入并生成预览，确认后才写入正式库；任意时刻取消只删试用目录，正式库零写入。中断后重新打开 App 会提示先处理未完成的导入。
- **导入设置随牌组走**：字段匹配、牌组到课程的摆放选择改存 Anki 库自带的配置区，备份 / 恢复 / 导出时自动携带；课程树改为可随时重建的派生视图，不再维护多份副本。
- **删除牌组真正回收**：先立即从课程树和复习中隐藏，后台再清理卡片与空间，「删了还在」「越删越大」成为历史。
- **大牌组导入不再卡界面**：导入全程后台进行并支持取消；真机完成强杀矩阵与大库性能验证。
- 「复习全部」支持跨多个 Anki 牌组连续续接；记忆保持曲线展示更准确；新增分节底部弹窗。
- 导入预览向导焕新：字段匹配确认与识别证据展示更直观。
- 新增朗读功能总开关，可一键关闭所有自动朗读；课程管理与学习页滚动结构优化，界面动效统一。
- 更新日志页新增「一键复制」，About 页内嵌的更新日志同样可用。

### 修复

- 修复导入完成页可能卡死闪退（返回键死循环）的真机问题。
- 修复上次导入未完成时仍可发起新导入导致的重复卡片。
- 修复启动维护被整体跳过、导入前引擎未打开等接线问题。
- 修复部分场景下课程树章节缺失或显示异常。

---

## 0.7.5 导入识别器重做 (2026-08-29)

### 改进

- 导入 Anki 的卡片识别推倒重来：不再逐张卡跑内容猜规则，改为读牌组自己声明的结构（卡片类型、模板正反面引用、{{type:}}/tts 过滤器、字段名词典、位置先验）一次判定整类卡片。同一类卡不会再出现「兄弟卡题型漂移」，识别开销与卡数解耦（2 万卡秒级 → 毫秒级）。
- 预览页每类卡片给出「为什么这么识别」的证据展开（哪个字段当正面、哪个当背面、依据是什么），并显示识别置信带（auto / 建议确认 / 需确认）。
- `{{type:}}` 拼写模板、选项池字段（A|B|C|D + 答案）、正面音频驱动等结构现在能被直接识别为拼写 / 选择 / 听音题型；字段词典只收结构性同义词（front/正面/答案……），永不收学科词。

### 修复

- 修复挖空卡片多删除点时复习到第二张仍挖第一个空：按卡片序号选取对应 `{{cN::}}` 删除点。
- 兼容存量：已确认的字段映射原样生效，旧映射 JSON 自动换算到新角色命名，零数据库迁移。

---

## 0.7.4 语言课与导入 Anki 分队列 (2026-08-29)

### 改进

- 练习页与个人页按课程类型切开复习入口：语言课只保留错题 / 单词 / 语法；自行导入的 Anki 课只保留 Anki 复习（并隐藏薄弱单词）。
- 导入 Anki 需先完成课程里的一课，该课卡片才会进入 Anki 复习；有导入历史的卡片可直接复习。课内第一遍不写官方排期、不进错题本；第一次「不认识 / 认识」发生在 Anki 复习。刚解锁的那一课，当天新卡上限至少覆盖该课张数（官方 `extend_new`，不改牌组预设）。重做已完成课：整课结束后对今日尚未官方评分的卡写成 Good/Again（提前复习）；中途退出不写；写入失败会在完成摘要里提示。

### 修复

- 修复 Anki 到期数长期显示「—」：官方复习队列拉取上限与原生 1–100 对齐，空队列不再把整次刷新标为不可用。
- 修复 Anki 复习第二张卡报「当前卡片无法安全写入」：评分后实时队列原地重建使批次列表缩短/重排，推进改由调度器刷新后的当前卡驱动（`advanceTo`），搁置/暂停/重做同步对齐，不再按 index+1 跳过队头。

---

## 0.7.3 练习页焕新 (2026-08-28)

### 改进

- 练习页主页全新信息架构：顶部「开始今日复习」主卡（今日总待复习大数字 + 错题/单词/语法/Anki 四队列速览 chips），短按一键智能进入最高优先级队列（错题 → 单词 → 语法 → Anki）。
- 复习入口收敛为「复习队列」2×2 网格（错题/单词/语法/Anki），消除旧版「今日重点 + 复习中心」中「复习」入口的重复。
- AI 助手入口合并为单行卡片，引擎就绪/未配置状态一目了然。
- 「练习工具」改为三列紧凑格：薄弱单词 / 词典 / 复习进度。
- 新增长按浮窗交互：主卡与错题/单词/语法/Anki/薄弱单词/复习进度/AI 入口支持长按，浮窗锚定在卡片旁弹出（缩放 + 逐行滑入动画），展示该入口的实时数据（待复习构成、最近错题、高频遗忘、Anki 牌组拆分、近 7 天复习柱状图、AI 引擎配置等），浮窗内可「进入」直达页面；词典与 Playground 不参与长按。
- 长按浮窗适配无障碍：随系统/应用内「减少动画」退化为纯淡入，支持屏幕阅读器长按语义（「长按查看数据详情」），触感反馈遵循「感官减负」偏好。

### 修复

- 修复退出 AI API 配置页时偶发的 `widget tree was locked` 框架异常：dispose 补存草稿改为微任务延迟通知，`notifyListeners` 不再落在卸载锁定期内；新增回归测试（含 provider 监听依赖的复现场景）。
...
- Anki 导入生成的课程树改按牌组名称自然排序（Unit 2 排在 Unit 10 之前），并始终排列在内置课程之后，不再随机交错；工程明细见 [doc 36](./docs/official-anki-migration/36-projection-tree-ordering-and-limits.md)。
- 超过单 section 60 个 unit / 单 unit 40 课的导入自动按 "Part N" 拆分，超大牌组不再生成打开即报错的损坏课程。
...
- 修复 Anki 课程与内置课程排序号冲突导致的顺序随机、每次启动不一致的问题。
- 修复内容版本重置后已导入 Anki 课程消失且不再重建的问题。
...
- Anki 导入全面重做：牌组先完整试装、预览确认后才真正加入课程，中途取消不留残余。
- 几万张卡的大牌组导入不再卡界面；导入完成时的卡死闪退已修复。
- 删除牌组真正「删干净」：卡片、课程条目和空间一起回收。
- 「复习全部」可跨多个牌组连续进行；记忆保持曲线更准确。
- 新增朗读总开关与分节底部弹窗；导入预览页面焕新。
- 学习页滚动更顺滑、界面动效更统一；清理两万多行旧代码，应用更小更稳。
