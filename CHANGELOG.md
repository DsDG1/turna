...
### Added
- AI 伴学：自由问答、学习诊断、讲解收藏、词典 AI 扩展、Anki 卡片讲解。
- 课内提示流式回复 + 学习者上下文注入（水平、错题、讲解偏好）。
- 土耳其语内置课程扩充：约 148 词、18 表达、8 语法点、54 课（A1→B2）。

### Changed
- AI Hub 重组为伴学优先（问答 / 诊断 / 收藏与创作入口分区）。
- 统一讲解偏好与 `AiErrorMapper` 友好错误提示。

### Removed
- 小艺（Xiaoyi）系统 AI 桥接；统一走本地 AI 引擎。

### Security
- 进度导出不再包含 `ai.engineConfig` / API Key。
...
### Added
- FSRS 连续记忆模型 + 本地参数优化器(替换 / 增强 SM-2)。
- Anki 智能牌组归类 + 牌型渲染重构。
- AI 引擎:`ai_cache` / `ai_cancel_token` / `ai_http_client` / 教材导入
  管线。
- 课程管理页、AI 中心(Xiaoyi)、AI 深度导师、应用图标 + 文案 + 本地化
  重写。
- 记忆曲线 / 复习进度 / SRS 学习导师面板。
- Anki 媒体条 + 牌型 AI 助手 + 复习进度页。
...
### Removed
- `lib/application/anki/anki_card_enhancer.dart`(合并到新设计)。
- `lib/l10n/app_*.arb` + `app_localizations*.dart` + `locale_provider.dart`。
- `anki_import_service.dart`(合并到 resolver + importer)。
- 陈旧 ADR 0021–0026 + `gui-beginner-guide.md` / `.html`。
...
- AI 课程设计器、教材导入、进度导出 / 导入。
- 独立设置页:主题、提醒、音效、无障碍、账户数据。
- 词典搜索、弱词复习、每日挑战、学习统计仪表盘。
- 暗色 / 亮色 / 跟随系统主题。

### Changed
- 目标语言由斯瓦希里语迁移为 **土耳其语**(TTS 代码 `tr`)。
...
### Removed
- hearts / leagues / 好友 / 商店 / 推送 等 11 项社交化、游戏化、付费功能。
...
### Added
- future4 框架完成:整洁架构收尾、DI 整合、音频 / 内容解耦、路由守卫。
...
### Added
- 离线优先重写:Firebase 下线,SQLite + Provider。
- Section → Unit → Lesson 层级 + 13 种交互题型。
- 6 种课型模板:intro / practice / listening / reading / review / mastery。
- SM-2 间隔重复(词汇 + 语法)+ 错题本 FIFO。
- 表达层 SRS(schema v5、Seeder、Repository、Provider、复习 UI)。

### Changed
- 顶级社交化 / 游戏化大面积删除(hearts / leagues / 好友 / 商店)。
- 全文中文化,设置与暗色主题集成。
...
上游 Varnamala 的初版(before fork)。

### Added
- 西班牙语 / 卡纳达语 625 词学习应用。
- 6 种题型:选择、翻译、填空、听力、配对、朗读。
- Match Madness 配对小游戏。
...
[0.0.1]: https://github.com/rshrc/Varnamala/commit/02cf739
