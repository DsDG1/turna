...
| 2026-09-04 | **Step 6.5 收尾清理**（零生产行为变化的死代码收敛）：① saga v1 提交半区删除（`commitLive`/孤儿 `finishCommit`/空壳 `_promoteMappings` + `Phase.projecting/publishing`）；② v1 恢复编排器整体删除（`official_anki_import_orchestrator.dart` 290 行——`recoverAttempt` 生产不可达：repair executor 只用 engine/sources、worker `state.orchestrator` 只写不读；`decideOfficialAnkiRecovery`/`OfficialAnkiRecoveryAction`/`OfficialAnkiFaultPoint`/`OfficialAnkiClock`/`OfficialAnkiNativeCommitState` 随之退役；repair executor 改直收 engine+sources，死类 `OfficialAnkiImportSagaCoordinator` 删除）；③ `allowsV2ImportChain` 恒真 getter + 8 处死守卫分支删除（两个 v2 读类的 `flagsOf` 注入缝一并移除）；④ v2 pending-imports 死模块删除（93+131 行，生产零引用；**决策：K1 续跑 UX 以 banner 的 discard-only 设计为准**）；⑤ course.db 新装库收敛——`_ensureAnkiCanonicalV2` 不再建 `anki_decks`/`anki_practice_projections`/`anki_import_issues`（修「新装带 3 张死表、升级库反而没有」的 schema 分叉，3 表补进 v2 链 Guard 禁用清单）；⑥ `anki_note_dao.projectionForCard` 恒 null 死读路径删除（含 `AnkiPracticeProjectionRecord` 与浏览器诊断对话框的恒假分支）；⑦ attempt 表新鲜 DDL 死列清理（`checkpoint_id`/`native_import_token`/`imported_note_ids_json`/`cursor_json`/`last_error_code`/`recovery_count`/`native_commit_state`/3 个 generation/`cleanup_phase`；**决策：v13→v14 回执不回填——恢复开关已删，无存活读者**）；⑧ 行级孤儿（`nativeImportToken`/`importedNoteIds`/`setNativeCommitState`/`commitIndexBatch`/`noteIdPage`/`replaceNoteIds`/`transition` 死参数/`unitCount`） | 完成 | `flutter analyze` `No issues found!`；`test/application` + `test/application/anki*` + `test/data` + `test/views/anki*` 全绿（review_dashboard/review_progress 2 项失败经 stash 基线复跑确认为分支既有问题）；open-before-import 顺序契约测试改挂 `OfficialAnkiV2ImportService.commit`（Step 1 task B 契约不变）；架构 Guard 对 v1 恢复编排器反转为「保持删除」断言 |
| | | | |
