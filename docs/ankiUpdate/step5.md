...
> **本步取消。整个 v2 已失败（2026-09-01）。** 总结论：[README.md](./README.md)。
>
> 取消范围：不为「老用户」搬家当；不做 census / 三样短事务 / 真机升级实测 / 观察期。没有 v1→v2 收敛，也没有为 Step 6 扫清数据面。下文是失败前写好的设计，**未施工**。
...
| 2026-09-01 | **取消；整个 v2 失败** | 未施工 | 不做存量迁移、不考虑老用户。无 census、无三样短事务、无升级实测。见 [README 失败结论](./README.md) |
