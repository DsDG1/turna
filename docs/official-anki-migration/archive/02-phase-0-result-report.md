...
| Flutter release APK 能否稳定加载？ | **未证明。** 旧 plugin-loader 错误已越过；最新本机失败为 Gradle daemon OOM（14 GiB、无 swap）。无 adb。 |
| open / import / render / queue / answer / undo？ | Host 已证明。真机 FFI 未证明。 |
| Unicode / Reverse / Cloze / 媒体？ | 冻结 fixture + golden HTML 通过。AV 现为官方 typed tag DTO，不是手工 `[sound:]` 扫描。 |
...
- 下一步：修 Gradle OOM / 真机加载，作为第二阶段第一张票
...
| ID | 风险 | 现状 | 对决策的影响 |
|---|---|---|---|
| R-APK | 本机无法稳定打出含新 `.so` 的 release APK | 最新失败为 Gradle daemon OOM（14 GiB、无 swap），旧 plugin-loader 错误已越过 | 挡住无条件 Go |
