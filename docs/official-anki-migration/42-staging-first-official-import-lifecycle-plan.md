...
> 日期：2026-08-31
>
> 状态：**P4 已施工**（2026-08-31）。P0 S-a/S-b/S-c/S-d 绿。生产导入只走 staging → `commitLive`。catalog v12。本计划取代 [doc 41](./41-official-anki-lifecycle-and-storage-remediation-plan.md) 的「导入链路修复」部分（S1 控制权、S2 启动恢复的导入分支、S6 checkpoint 体系对新导入全部作废），保留 doc 41 的「删除与物理回收」部分（S3/S4/S5/S7/S8）作为独立存量善后。完成状态以本文 §10 门禁与 §12 收据为准。
>
...
| P2 | `5cf0fb68` | `flutter test test/views/anki_official/official_anki_mapping_page_test.dart test/application/anki_import/official_preview_mapping_intents_test.dart test/application/anki_official/official_anki_projection_scope_p0_test.dart` → **S-d + skip/needsMapping + mapping 回归 passed**；S-c 仍红。S-a/S-b 保持绿。 | 映射页二选一正面、save 恒可点并 pop；preview「已确认」；skip 清横幅；确认不写全局 mapping 表；`OfficialAnkiMappingRoute` 移出 diagnostics guard。 |
| P3 | `93ceac3f` | `flutter test test/application/anki_official/official_anki_projection_scope_p0_test.dart test/application/anki_official/official_anki_commit_live_test.dart test/views/anki/anki_import_staging_first_p0_test.dart test/views/anki_official/official_anki_mapping_page_test.dart` → **S-c + commitLive + S-a/S-b/S-d passed**。 | `projectSource(notetypeIds:)`；`commitLive` 写 live Collection 并晋升 mapping；`OfficialAnkiStartupRecovery`；pending 导入条。`importFile` 仍保留（P4）。 |
| P4 | `5c872277` | `flutter test test/application/anki/official_anki_architecture_guard_test.dart test/application/anki_official/official_anki_import_orchestrator_test.dart test/application/anki_official/official_anki_recovery_test.dart test/application/anki_official/official_anki_lifecycle_storage_test.dart test/application/anki_official/official_anki_commit_live_test.dart test/application/anki_official/official_anki_staging_manager_test.dart test/views/anki/anki_import_staging_first_p0_test.dart test/application/anki_official/official_anki_projection_scope_p0_test.dart test/views/anki_official/official_anki_mapping_page_test.dart test/application/maintenance/storage_inventory_service_test.dart` → **guard + orchestrator/recovery/lifecycle/inventory + S-a/S-b/S-c/S-d + commitLive/staging manager passed**。 | 删 live-first `importFile`/`createBackup` 预览路径；wizard/`importThenPreview` 只 `startStaging`；`restoreBackup` 仅存量 checkpoint；staging 库存分项；worker `importFile` 薄转 `importPackage`。 |
