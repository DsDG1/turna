...
> 日期：2026-08-31
>
> 状态：**P1 已施工**（2026-08-31）。P0 S-a/S-b 复绿；S-c/S-d 仍红（P2/P3）。catalog v12。本计划取代 [doc 41](./41-official-anki-lifecycle-and-storage-remediation-plan.md) 的「导入链路修复」部分（S1 控制权、S2 启动恢复的导入分支、S6 checkpoint 体系对新导入全部作废），保留 doc 41 的「删除与物理回收」部分（S3/S4/S5/S7/S8）作为独立存量善后。完成状态以本文 §10 门禁与 §12 收据为准。
>
