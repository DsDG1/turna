Generated: 2026-08-25 (doc 34 ANKI-CUTOVER-ONE-SHIP 一次性施工，OS-00–OS-24 host 门禁)：CourseDatabase v21（`anki_course_sources` owner 权威列 + `anki_owner_transitions` in-flight 唯一索引 + `legacy_pending_migrations` + scope repair journal）；`CourseScope` 类型化 codec（`course-scope:v1:*` wire key、percent-encoding、legacy `anki:src` 歧义回 builtin）+ 启动幂等偏好修复（locator 在 DB open 后、Provider 加载前执行）；CourseProvider 目录化（`CourseCatalog` 读 anki_course_sources + manifest 兜底、builtin 严格排除 Legacy+Official section、完整 src-<id> round-trip、lastIndexOf('-s') 解析取代 split-first）；课程管理页精确卸载（展示 shortId+卡片数、完整 sourceId 删除、高亮新导入）；顶栏显示当前课程名；导入完成页三动作分流（完成保持原课程/立即学习才切换/查看牌组高亮）；正式复习 fidelity-first（markup→FidelityCardPresentation，纯文本才 Flip）+ `OfficialFormalReviewLiveQueue`（每次 answer 后按 Scheduler 实时队列重建 items、快照 generation 失效、Again 重插保留）+ Review All 聚合全部 Official 来源（openDueDeck + 卡集并集 + per-source 失败上报）+ Redo/Bury/Suspend 头部动作；`OfficialFormalDueRepository` 六集合快照（known/unknown/unavailable 三态、generation 防 stale、rollback 保留旧六集合、retired 接 placement active=0 真实 reader）+ `OfficialAnkiHomeDue` 降为薄 facade（officialDue 派生不再可写）；`LegacyWriteFence` 全 mutator 写围栏（AnkiNoteDao 14 个 mutator + ledger undo；normalLegacyWrite/migrationCleanup token/rollbackRestore intent；authority 转换即时发布、legacy import id 经 transition 链继承 fence）；W8 saga 接权威（freeze=open→frozen 真实 CAS + phase 机快进、owner switch 权威先行 catalog 为 mirror、driver 补 resume/rollback 入口）；official-first 导入写权威表（import 起 staging、publishFromProjection 后 commit-last active）；Browser/Stats 官方来源按钮可达（页面原有 Official 分支按完整 sourceId 路由）；`TurnaMigrationImporter`（zip magic/zip-slip/manifest/SHA256SUMS/容量 fail-closed、事务应用、Legacy Anki 行只入 legacy_pending_migrations）+ 设置页独立迁移包入口；`DiagnosticsReleaseGuard`（6 条 Official 内部路由 release+深链阻断、diagnostics 显式 opt-in）。native pin `967aa0d57` + 3 补丁 verify_pin 通过、`build-android/build.sh` 重建 libturna_anki.so（符号校验 pass，sha256 见 cutover-once/so-sha256.txt）。新增测试 16 文件 + 既有测试对齐新 API/派生语义。`flutter analyze` lib：0 error、88 warning/info（原 133）。全量 `flutter test --exclude-tags golden`：**1825 passed / 0 failed**——包括修复 3 个既有基线失败（wetland_palette_contract 结构正则、ai_api_config dispose 草稿 flush、dark_mode_text_contrast 缺 AccessibilityProvider）与 2 处 schema 字面量回归（remote_backup 硬编码 8/20）；golden course_tree+play_hub 4/4。release 真机矩阵与 OHOS 外部事实未做（证据项保持未勾）。
