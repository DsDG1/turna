...
Generated: 2026-09-22（Anki 导入全量优化）：同 sha256 的 active 包短路为「已导入」；`preview_ready` 且 staging 完整时可继续预览。契约 minor 13，新增 `SUMMARIZE_IMPORTED_NOTES`（43）与 `PROMOTE_STAGING_COLLECTION`（44）；预览按牌组×题型汇总、样卡懒加载、学习方式按题型、blocking 行可跳过、牌组可勾选排除出课程树。提交优先把 staging collection 提升进 live，失败才回退 `importPackage`；完成页写新增/跳过、拆分与媒体状态。**新测** 预览 +5、controller +3、v2 链 +3。**验证**——定向 `flutter test`（preview / controller / official-first / staging / open-before-import / v2 chain / contract integrity）通过。全量套件未重跑。

Generated: 2026-09-21（末卡写入失败不再进完成页）：`ReviewSessionController` 最后一张卡等 ledger persist 成功才 `isComplete`；失败留在当前卡可 `retryWrite`，重试成功也会跑 `onSessionCompleted`。**新测** P4 +3（persist 中不完成 / 失败可重试 / retry 结算）。

Generated: 2026-09-21（UX 残留小批次）：Learn `DueChip` 在 Anki 课读官方 due 且 Home 刷新；`ReviewLedger.answer(cachedPreview:)` 去掉 Turna/官方答卡重复 preview；其余 AI provider 存 `AiErrorMapping` 并接 `AiErrorBanner`；`relearnPreviewLabel` 迁 AppStrings；`PracticeSessionBody` 接入课时/每日/弱词/错题重做。**新测** ledger cachedPreview + relearn 标签。**验证**——`flutter analyze` **No issues found**；全量 `--exclude-tags golden` **1861 passed / 0 failed**（1859 + 2）。

Generated: 2026-09-21（UX 体验改良施工计划 B1–B7）：对照桌面施工计划落地课时土耳其语判分、错题刷新、星期标签、复习会话（退出确认/乐观写入/分批/撤销条/预览错误分流/自动朗读）、启动与首页钩子（跳过 Splash、Get Started 加载、due chip、next-up 展开、弹窗预算；B3-6 seed 异步化仍按计划跳过）、课时快赢、AI 反馈面（可选 Markdown 与聊天持久化中期项跳过）、危险操作与 B7 清扫。**新测** `turkish_text_test` / `weekday_label_test` / `official_schema_for_preview_node_test`，并补渲染器 İ 判分与 InteractionState 导入。**验证**——`flutter analyze` **No issues found**；全量 `--exclude-tags golden` **1859 passed / 0 failed**（1842 + 17）。

Generated: 2026-09-21（性能批次 2：写放大/内存/主 isolate 六项）：**错题本增量持久化**——`MistakeRepository` 新增 `insertEntry`（FIFO 尾插，`MAX(sort_order)+1` 防序号碰撞，`evictOldest` 一并淘汰头行）/ `updateEntry`（原地单行更新，`sortOrder` 用 `Value.absent` 防止改写序）/ `deleteEntriesByIds`（单语句 `isIn` 删除）三方法，aggregates 由调用方传入缓存值做单行 upsert；`MistakeProvider` 五个热路径（record / recordRewrite 两分支 / removeByIds / removeForAnkiDeletion）改走增量，`clear` 与迁移/恢复路径保留 `replaceAll` 全量重建（prefs 降级模式不动，`Anki 卸载匹配谓词提取为 _matchesAnkiDeletion`）。此前每答错一题 = DELETE 全语言行 + 重建 ≤200 行。**新文件** `test/data/mistake_repository_test.dart`（+6：FIFO 跨删除间隙有序 / 淘汰仅头行 / 更新不改序 / 删除限定 id+写聚合 / 增量与 replaceAll 收敛一致 / 语言隔离）。**课程 seed 批量化**——`course_database_seeder` 的 `_seedSections` 三重循环逐条 `await insert` 改每 section 事务内 units / lessons / lessonContents 三个 `db.batch`（照同文件 vocab 样板；`.turnapack` 导入 ~9300 课路径从逐课两次 isolate 往返降为每表一次）。**Anki 媒体图片解码上限**——`anki_media_strip` 的 `Image.file` 补 `cacheWidth/cacheHeight`（CachedAssetImage 同款 `clamp(0,600)×dpr` 公式），牌组多百万像素照片不再全分辨率解码。**resolveFile 结果缓存**——`CoursePackMedia.resolveFile`（platform channel + 同步 `existsSync`，选项 tile 高频重建反复触发）加 per-path memo（key 含 persist 根标识，负结果也缓存），`deleteExtractedMedia` 内部失效 + importer 暂存目录换入后失效；`course_pack_media_test` 新增缓存失效契约用例（+1）并给既有用例 tearDown 补清缓存。**telemetry 字节数近似**——`study_log_repository._writeString` 与 `mistake_provider` prefs 降级的 `utf8.encode(encoded).length` 改 `encoded.length`（字符数近似，去掉每次写 prefs 的整串 UTF-8 再分配）。**字典搜索防抖**——`dictionary_page` onChanged 改 250ms Timer 防抖（anki_card_browser 同款），dispose 取消；golden 测试用 `pumpAndSettle` 推进假时钟不受影响。**验证**——`flutter analyze` **No issues found**；`dart format lib test` 0 changed；全量 `--exclude-tags golden` **1842 passed / 0 failed**（1835 + 7）。

Generated: 2026-09-21（P0 正确性 + 卫生批）：**AI Provider 双实例修复**——`providers.dart` 的 AiCourse / AiWish / AiGroundedResource / AiLessonHelper 四个 provider 由裸构造改 `getIt<T>()`（此前 UI 树与 DI 容器各持一份：SrsTutorProvider 经 getIt 写 `AiCourseProvider.saveSectionJson`，helper sheet 监听的是树实例，tutor 写入 UI 收不到通知）；`AiCourseProvider` / `AiWishProvider` / `AiLessonHelperProvider` 构造兜底 `?? AiGroundedResourceProvider()` 改 `?? getIt<AiGroundedResourceProvider>()`（旧写法在树路径再 new 出第三份 grounded）。TextbookImportProvider 无 getIt 注册且无服务侧消费者，保持树构造（注释说明）。`provider_identity_test` 新增「AI providers on the tree are the same instances services receive」守卫用例（+1，防复发）。**SRS 写放大修复**——`registerItem` / `registerAllItems` 持久化从全队列 `upsertBatch` 改为只写新增行（导入万卡队列后每答一题不再重写整队列；同时把注册写与紧随的评分单行写收敛为同键两写、顺序确定）；`removeImportedItems` 循环逐条 `delete` 改新增 `SrsStateDao.deleteMany` 单语句 `isIn` 删除。受影响测试两处（`ai_course_language_write_test` / `srs_tutor_provider_test` 的 `_RecordingAiCourseProvider`）改为显式传 `groundedProvider:`（partial-DI 套件不注册 grounded 单例，构造兜底会抛）。**卫生**——`git rm --cached` 两个 logcat 取证文件（约 1.1MB；`.gitignore` 规则早已存在，只是对先跟踪文件不生效）；`discord_notification.yml` shell 注入面修复（github.actor / commit message / PR title 从 `run:` 内插改 env 传入 + jq 构造 JSON，补 concurrency 与 `**.md`/`docs/**` paths-ignore）；`build.gradle` 显式钉 `minSdkVersion 24`（对齐 Rust 内核 API 24 链接地基，防随 Flutter 默认值漂移）+ launcher icons `min_sdk_android` 21→24 同步；pubspec 移除未接流程 dev 依赖 `import_sorter` / `pubspec_dependency_sorter` / `flutter_gen`（CLI 包，保留 runner）；`build_release.py` 新增 `--version` 与 pubspec `version` 的 X.Y.Z 一致性断言（预发布后缀可不同，`--allow-version-mismatch` 逃生），阻断产物名与 About 页版本静默分叉。**口径修正**——上批（2026-09-20）所记「GUI 1276 项首次进 CI」为过期数字（2026-08-05 前的旧基线），GUI 现行基线以 `tool/gui/tests/BASELINE.md` 的 **2541 例**（2026-09-20）为准。**验证**——`flutter analyze` **No issues found**；`dart format lib test` 0 changed；全量 `--exclude-tags golden` **1835 passed / 0 failed**（1834 + 1）。

Generated: 2026-09-20（CI 补强与格式归零批）：**dart format 归零**——`dart format lib test` 重写 15 文件（12 lib + 3 test，纯格式无逻辑变更），`format-check` 本地与 CI 由红转绿。**CI 补强**（`flutter_ci.yml`）——顶层 `permissions: contents: read`；Flutter 钉 `3.44.5`（对齐本机绿版本，另 4 个 workflow 同步钉版/加权限）；`setup-python 3.12`；新增 format-check step（codegen 前只查已提交源码）；`flutter test --coverage` + lcov 汇总与 artifact 上传；新增 apt 无头 Qt 库 + `pip install ./tool/gui[dev]` + `ruff check tool/gui/src`（src 现状 0 违规；tests 有 139 笔历史债另批，未门禁）+ `run_gui_tests.py full -j 4`（GUI 1276 项首次进 CI）。**新文件**——`.github/dependabot.yml`（github-actions/pub weekly + pip monthly，刻意不含 gitsubmodule：anki 钉 spike detached commit 不跟上游）、`.github/workflows/gui_release.yml`（`gui-v*` tag / dispatch → windows-latest ci 档门禁 + PyInstaller 出 `turna-gui.exe` 挂 Release）、根 `SECURITY.md`。**Makefile**——新增 `lint-gui`/`test-gui` 并入 `ci` 目标对齐 GitHub CI。**卫生**——删根目录 `nul` 设备名残留与 `pubspec_overrides.yaml.disabled`（均 untracked，后者全仓零活引用），`.gitignore` 补 `nul` 条目。**文档修正**——project-guide §14 清除陈旧记录：两个「历史环境敏感测试」实已退役/重写（`anki_review_fidelity_test` 随复刻层删除，`anki_note_dao_test` 已按 doc 38 P1-B 重写），GUI 测试命令更正为 `run_gui_tests.py` 入口（裸 `unittest discover` 缺 sys.path 自举不可用）；CLAUDE.md 开发命令同步。**验证**——`flutter analyze` **No issues found**；全量 `--exclude-tags golden` **1834 passed / 0 failed**（计数不变）；首次记录总行覆盖率 **58.3%**（31599/54247，`coverage/lcov.info`）；根 Python 测试退出码 0；GUI full 档本地未完成（负责人中止，首次完整执行将由 CI 承担，如 CI 过慢可降 `ci` 档一行回退）。

Generated: 2026-09-17（测试套件冗余清理批 −24）：**恒真/无效断言清除**——删 `firebase_options_test` 整文件（镜像断言 FlutterFire 生成常量 + 断言生成代码在 iOS 抛错，Firebase 仅 Android 初始化）；删 `provider_identity_test` 用例 3（名义测 LessonViewModel 注册，实断言与用例 1 重复的 Course/Mistake）；删 `locator_test` 用例 4（`returnsNormally` 与用例 1/3 重复的 Phase 3 占位）及用例 1 中测自身注册的 `isRegistered<AppPrefs>` 断言；删 `bottom_navigator_test` `overlayExtent` 用例（断言即 `static const` 定义式本身）；`turna_select_test` 前两用例点击后原断言的是点击前就存在的文本，补 `selected == _Mode.b` 与 `SegmentedButton.selected` 真断言；`official_anki_composition_test` `allowInProcessFallback` 用例原为零 expect 恒绿，且目标 fallback 分支经 `requireImporter` 不可达（flag 门禁先抛 `flag_fail_closed`）——改写为钉住「flag gate 先于 fallback」的真实行为。**重复用例清除**——`course_domain_models_test` 的 `interactionGrammarPointId` 与 `interaction_test` 逐字重复删一边。**生成代码/死 API 用例清除**——`course_domain_models_test` 删 LessonWordLink（freezed serde + `@Default`）与 MistakeEntry JSON round-trip 两组；`core_utils_test` 删 `Result.guardSync`/`map`/`flatMap`/`fold`/`logFailure` 四组（全 lib 仅 `Result.guard` 一处调用点，Result 为渐进采用预留）；`local_user_avatar_test` 删 avatarId getter 与两个纯 `x ?? this.x` copyWith 用例（保留手写 serde 向后兼容与 `clearAvatarId` 分支）；`text_styles_test` 删 `buttonLabel` 常量断言；`official_anki_official_first_flags_test` 删 copyWith round-trip 用例与构造默认值断言。`dart analyze` lib+test **No issues found**；全量 `--exclude-tags golden` **1834 passed / 0 failed**（1858 − 24；golden 12 个未跑）。

Generated: 2026-09-15（UI 审计收口批：本地化/无障碍/主题/文案收敛）：**本地化**——`pubspec` 补 `flutter_localizations`（sdk）+ `intl` 已有；`MaterialApp.router` 补 `localizationsDelegates`（Material/Widgets/Cupertino Global delegates）+ `supportedLocales [zh,en]`，系统级 UI（timePicker/选择菜单/tooltip）随系统语言。**无障碍**——根 `textScaler` 由覆盖改为「系统 × App 内滑杆」相乘叠加，OS 大字体不再被压回 100%；底部导航胶囊标签 textScaler 封顶 1.4（200% 缩放不再撑爆 64px 胶囊）；`Loader` 橙改 `brandTeal`；`info_popup` 遮罩补 `Semantics` 关闭标签；~15 处缺失 `IconButton.tooltip` 补齐。**主题**——4 套主题（light/dark/HC×2）统一补 `filledButtonTheme`（radiusMedium 圆角收口 M3 默认体育场形）与 `dialogTheme`（radiusLarge + 高对比描边）；新增 `TurnaTheme.errorOnSurface`/`accentOnCard` helper。**深色可读性**——transparency_log 堆栈容器/错误色走主题感知；privacy_details callout 用 `accentOnCard`。**SnackBar 收敛**——`TurnaSnackBar` 补 `maybeShow`/`showVia`（messenger 预捕获跨 await 变体），views 层 ~60 处裸 `ScaffoldMessenger` 全部迁移；`application/` 层 2 处受分层守卫限制保留裸 API（由 `snackBarTheme` 统一外观）。**字符串收敛**——`AppStrings` 新增 ~120 键（`aiGuide*`/`systemHealth*`/`settingsMigration*`/`ankiPreview*`/`mcqPreview*`/`ankiReview*`/`reviewSource*`/`cosmeticsVoucher*`/`insights*`/`aiTaskLabel*` 等分组），views 层中文硬编码字面量从 ~284 处收敛至 ~40 处且余量均为标点占位符（'—'/'·'/'、'）或诊断报告 payload；`system_health_page`/`ai_feature_guide_page`/`data_backup_settings_page`/`course_management_page`(TTS 语言标签 `ttsNativeLanguageLabel`) 等大户清零。`textScalePercent` 能力契约保留（`accessibility_provider_test` 钉住）。`flutter analyze` lib+test **No issues found**；全量 `flutter test` **1870 passed / 0 failed**。

Generated: 2026-09-15（官方 Anki 导入优化批：A 正确性 + B isolate + C 廉价收益 + D2/D3）：**A1 staging session 泄漏**——`finishCommit` 现在先 `manager.kill()` 再删 staging 目录（此前只有 abandon/cancel 会 kill，成功提交后 worker isolate 永存、每导入一次泄漏一个）。**A2 cancel-during-commit**——`cancelActive` 明确排除 `committing`/`receiptCommitted`（`stagingPath` 因 COALESCE 永不清空，兜底条件会误伤进行中的提交）；controller `cancel()` 在 `_commitInFlight` 时不再跑 `_cancelStagingSaga`；Committing 步取消按钮移除（不可中断就不该显示）。**A3/A5 preview 取数修正**——`preparePreview` 改从 staging 引擎按 receipt `associatedNoteIds` 批量取卡描述符算 notetype/deck/每牌组卡数（live catalog 的 `anki_source_cards` commit 前恒空，旧实现每牌组显示 0 卡）；staging engine 缺失直接 `capabilityMissing`，不再静默回落 live 引擎。**A4 错误掩盖**——`mapOfficialErrorToHuman` 只在 `debugDetails==unfinished_import_blocks_new` 或 messageKey 匹配时给 pending 提示，不再无差别改写。**B**——rebuild 读侧 offload 到 spawn isolate（`OfficialAnkiDatabase.filePath` 标记，内存库回落 inline）；`_topDeckId` 改 topName→deckId map；`skip/take` 翻页全改 `sublist`；`deleteDirectory` 递归删除包 `Isolate.run`；due-sync 收养改 active join + `listCardDeckPairs` 轻量列。**C**——hash 与 staging acquire 并行（acquire 内补 discard 自检防 spawn 竞态泄漏）；`hasUnfinished()` 用 `SELECT 1 LIMIT 1` 替代 `SELECT *`（不再物化 MB 级 receipt JSON）；`unfinishedBySource` SQL 过滤；source hash 经 `OfficialAnkiImportResult.sourceHash` 透传；视图 section/lesson 计数从内存 rows 算；`_writeDerivedPlacements` 一次批量 `readDeckPlacements`。**D2**——commit 后 `rebuildSource(sourceId)` 经 `replaceScoped` 只换该 source 的视图行（同事务顺手清非 active 残留），成本 O(变更) 替代 `replaceAll` 全表重建。**D3**——controller 轮询 `latestProgress`（独立控制通道）把原生阶段名透到 Parsing/Committing 的 `stage` 字段。D1（staging→live 合并省二次导入）需 rslib 改动，纯 Dart 不可为。测试 +3（cancelActive 跳过 committing/receiptCommitted、finishCommit 销毁 session+删目录、preparePreview 无 staging engine fail-closed）；`flutter analyze` 全库 **No issues found**；全量 `flutter test`（含 golden）**1860 passed / 0 failed**。

Generated: 2026-09-15（bug 修复批·续：due 计数 getter 有效期 + revision 漏 bump + reload 竞态收尾）：**due 计数/集合 getter 绕过有效期窗口**——`primaryDueCount`/`primaryCachedDueCount`/`dueCount`/`expressionDueCount`/`dueWordIdSet` 直接返回缓存值不看 `[computedAt, validUntil)`，挂起期间跨过 dueAt 的卡在所有徽标计数（play_hub/profile/course_tree/grammar/ledger dueSummary）中继续隐藏直到下次写入；`primaryDueCount`→`getDueItems().length`（命中时本就 O(1)）、`dueCount`→`getDueWords().length`、`expressionDueCount`→`getDueExpressions().length`，`dueWordIdSet` 改用 due 列表实例 `identical` 判新鲜（窗口内实例稳定保 `context.select` 安静，过期自动重建）；删 `_cachedDueCount`/`_cachedExpressionDueCount` 冗余字段。**ReviewDataRevision 漏 bump**——非 insert 的 review 数据变更此前不失效 revision 键控缓存：`undoReview`（撤销的事件仍计入留存/热力图）、`reloadFromStorage`（恢复/快照/推迟/包重导入后统计停滞）、`removeItemsByPrefix`/`removeImportedItems`/`importStates`/`registerItem`/`registerAllItems`/`setItemFlags`/`persist`/`clear` 全补 bump；`AnkiImportCleanupService` 新增可选 `dataRevision`，`uninstallDeck` 经 getIt 惰性解析接线，saga 提交后 bump。**同语言 reload 竞态**——`reloadFromStorage` 加 `_loadGeneration++`：在途 ensureLoaded 的旧 hydrate 结果不再 putIfAbsent 复活刚被 prune 的行；`_doReviewItem`/`undoReview` 守卫从仅 `_languageFilter` 扩为 generation+filter 双判，同语言 reload 换缓存后 grade/undo 改为把结果合并进活 map 而非 `_commit` 陈旧 pre-reload map 整段覆盖（修内存态分叉，DB 语义不变）。**ensureLoaded latch**——原在 await 前置 `_loaded=true`：加载失败永久空队列不重试、并发调用方早退读空态；改 `_loading` future 锁存 + `_loadingGeneration` 代际标记（并发并入同代 in-flight、换代自开新 load、失败 `_loaded` 保持 false 允许重试）。**次要**：`MistakeProvider.dailyCounts`/`masteredTotal` repo 模式未水合时不再读已清零的 prefs 残值并缓存（对齐 `entries` 守卫）；`SrsStateDao.delete`/`deleteByPrefix` 加 `queue` 作用域（PK `(language_code,word_id)` 无 queue 列，裸删会跨池误伤同 id 行），调用方传 `queueId`；`LocalReminderService._ensureTimezones` 的 `_tzReady` 移到 tzdb 初始化成功后（初始化抛错不再永久钉 UTC）；删死代码 `ReviewHistoryDao.deleteByLanguage`（卸载走 `deleteBuiltinLanguage` 事务，无调用方）。**顺带修预存 flaky**：`review_dashboard_repository_test` 的 `_seedDueCard` 用 `now-2h` 在 00:00–02:00 时段落入昨天被判 overdue，dueAt 钉当天本地零点（恒今日到期）。新增测试 13（queue_provider +6：计数有效期/undo bump/reload bump/ensureLoaded 重试/并发并入/stale hydrate 丢弃；srs_provider +2：dueWordIdSet 窗口+实例稳定/dueCount 双轨过期重算；race +2：同语言 reload 下 grade/undo 合并入活缓存；dao +2：delete/deleteByPrefix queue 作用域；mistake +1：repo 未水合不缓存 prefs 残值）。`flutter analyze` **No issues found**；全量 `--exclude-tags golden` **1845 passed / 0 failed**。

Generated: 2026-09-15（bug 修复批：每日提醒定时生效 + 复习统计口径 + 撤销误计掌握 + preview 死锁 + HTML 二次反转义 + due 缓存有效期）：**每日提醒**——`LocalReminderService.scheduleDaily` 收 `TimeOfDay` 却用 `periodicallyShow(RepeatInterval.daily)`（该 API 无时间点参数，用户选的 19:00 被静默丢弃、按调度时刻每 24h 重复）；改 `zonedSchedule` + `matchDateTimeComponents: DateTimeComponents.time`，新增 `timezone ^0.10.1`（原传递依赖转正）与 `flutter_timezone ^5.1.0`（设备 IANA 时区），`_ensureTimezones` 幂等初始化 tzdb、IANA 失败按 UTC offset 兜底。**复习统计口径**——表达式经 `registerItem` 默认注册为 `SrsSourceKind.builtin`，其 review_events 在"课程"来源过滤（`source_kind='course'` 单值）下从留存曲线/活动热力图消失、却计入来源总数；`ReviewHistoryFilter` 新增 `sourceKinds`（生成 `IN(...)` 谓词），course 来源匹配 `{course, builtin}` 与 `sourceReviewCounts`/`_allTagged` 的 rollup 对齐。**撤销误计掌握**——`unified_review_page._onOutcomeUndone` 走 `removeByIds`（语义=复习答对→掌握）给 masteredTotal +1；加 `countAsMastered` 参数（默认 true 保 mistake_review_page 语义），撤销路径传 false。**preview 死锁**——`ReviewSessionController._loadPreviews` 失败置 `_lastError` 使 `BinaryRecallBar` 禁用且提示"请重试"却无重试入口（首张卡无 receipt 时撤销也不可用→会话卡死）；新增 `retryPreviews()`，错误条补「重试」`TextButton.icon`。**HTML 二次反转义**——`stripHtml` `&amp;` 先于 `&lt;` 解码导致 `&amp;lt;` → `<`，改最后解码 `&amp;`。**due 缓存有效期**——`getDueItems` 主缓存原对任意更晚 cutoff 复用（挂起期间跨过 dueAt 的卡不显示）且跨过滤参数共享（`getDueItems()` 无参会污染 `getDueWords`）；拆出 `computeDueItemsDetailed`（返回 due + 最早未到期 dueAt），缓存按 `(typeFilter, excludeAnki)` 键控、仅 `[computedAt, validUntil)` 内复用，`getDueExpressions` 副缓存同机制。新增测试 9：srs_queue due 缓存 +3（未来到期刷新/更早 cutoff 重算/参数不串缓存）、mistake removeByIds countAsMastered +1、dao sourceKinds IN +1、controller retryPreviews +1、新建 `test/core/html_stripper_test.dart` 3。`flutter analyze` 全库 **No issues found**；`dart format` 改动文件已合规；全量 `flutter test --exclude-tags golden` **1832 passed / 0 failed**（1823 基线 + 9）。

Generated: 2026-09-13（代码治理 批6 / 治理尾巴收尾：course_tree 核查更正 + 同步盲区 unawaited + Section 类型化）：**尾巴1（course_tree 无需拆，更正核查结论）**——治理效果核查曾报告「`_buildRevealSlot` ~190 行 / `_buildSectionScrollView` ~175 行未拆」，实为测量误差：按方法起止精确复核，两方法现各为 39/53 行（09-08 展开性能批次已拆，批 2 BASELINE 记录的行数正确），原核查把 401–590 / 152–326 的多方法区间（含 `_resolveStatusProjection`/`_toggleUnit` 等）当成单方法长度；全文件最大方法 `_buildLoadedSlivers` ~120 行 < 200 验收线，零代码改动。**尾巴2（unawaited_futures 同步盲区）**——该 lint 仅在 async 上下文生效，`anki_card_browser_page` 3 处同步上下文裸 fire-and-forget 补 `unawaited()`：`initState` `_load()`（:68）、清除搜索按钮 `_load()`（:303）、`_OfficialPreviewText.initState` 预览懒加载 `_load()`（:622）。**尾巴3（视图层 dynamic 转型）**——`anki_review_screen` `_reorderDecks`（裸 `List`）与 `_aggregatedDue`（`List<dynamic>`）参数改 `List<Section>`（补 `domain/course/section.dart` import），消除 `(section as dynamic).id as String`×1、`section.id as String`×1 等 dynamic 转型，调用点 `allSections.where(...).toList()` 本就产出 `List<Section>`，行为零变化。`dart analyze` lib **No issues found**；`dart format` 改动 2 文件零净变更；定向 `anki_review_hub` + `formal_review_launcher` **10 passed**；全量 `flutter test --exclude-tags golden` **1823 passed / 0 failed**。

Generated: 2026-09-13（代码治理 批5 / P3-1+P3-2+P3-3：命名约定 + import 顺序 + 空 catch 注释）：**P3-1**——CLAUDE.md 工程约定新增「命名与目录约定」小节：页面统一 `*_page.dart`（存量 48 vs `_screen.dart` 9，新代码不得新增 `_screen.dart`，存量 anki/lesson/play/profile/review 系改名牵连路由与测试、渐进迁移）；组件目录定规则「跨功能共享件 → `lib/views/widgets/`、功能域私有件 → `<feature>/components/`」，新功能域目录统一 `components/`，存量 `profile/widgets`/`settings/widgets` 渐进迁移。**P3-2**——`lesson_viewmodel.dart:7` 与 `official_anki_engine_fake.dart:2` 的 `dart:` 导入移回文件头部标准位置。**P3-3**——全库扫描 `catch` 529 处（含 `catch (e)`/`on X catch`），空体且无注释 46 处（计划口径 lib 25 + test 21）全部补一行语义注释，与既有 `catch (_) {/* best-effort */}` 风格对齐：lib 侧按语义分类（optional GetIt 依赖守卫 ×8、best-effort dispose/close/cache-clear ×7、扫描中文件消失容错 ×3、JSON 解析兜底 ×1、provider 可选装配 ×5），test 侧 20 处统一 `best-effort temp cleanup`（tearDown 临时目录删除）+1 处 fixture seed；`anki_html_card_view` 内嵌 JS `catch (e)` 同步补注。`dart analyze` 全库 **No issues found**；`dart format` 改动文件已合规；全量 `flutter test --exclude-tags golden` **1823 passed / 0 failed**。

Generated: 2026-09-13（代码治理 批4 / P2-2+P2-4：anki 文案收敛 + unawaited_futures lint）：**P2-2**——计划枚举的 5 个文件用户可见中文文案全部收敛进 `AppStrings`（`anki_card_browser_page` 12+ 处、`official_anki_repair_center_page` ~20 处、`storage_diagnostics_page` ~20 处、`anki_deck_stats_page` ~20 处、`ai_feature_guide_page.dart:33` 标题）：`app_strings.dart` 新增 ~70 个键（`ankiBrowser*`/`ankiStats*`/`ankiRepair*`/`storageDiag*`/`commonDelete`/`aiFeatureGuideTitle`），复用既有 `commonClose`/`commonCancel`/`commonRetry`/`commonGotIt`/`databaseDoctor*`/`storageMediaFilesTitle` 等通用键；`deck_stats` 顺带修 `retentionPassed`/`retentionSample` 可空字段插值（原 null 会打印 "null"，改 `?? 0`）。枚举文件 grep 裸中文字面量 **0**（验收达成）；`ai_feature_guide_page` FeatureSpec 正文 70 处为该文件设计上有意留待 i18n 的内容，不在 P2-2 枚举范围；anki 模块其余文件（import_wizard/mapping/review_session 等 ~100 处）为后续批次候选。**P2-4**——`analysis_options.yaml` 启用 `unawaited_futures`（info 级，计入零问题基线）：全库暴露 24 处，分类修复——`Map.remove` 返回 Future 丢弃 6 处（course_provider 2、course_loader 4）与刻意 fire-and-forget（showDialog 2、router.push/maybePop 6、provider 触发/状态刷新 4）统一 `unawaited()` 显式标注；测试侧真补 `await`：`game_provider_test` `sub.cancel()`、`smart_speech_gate_test` `getIt.reset()`、`course_provider_test` `selectUnit/selectLesson` 改 `unawaited`（同步断言语义不变）。`dart analyze` 全库 **No issues found**；`dart format` 改动文件已合规；全量 `flutter test --exclude-tags golden` **1823 passed / 0 failed**。

Generated: 2026-09-13（代码治理 批3 / P2-1+P2-3：死代码删除 + 文档/注释漂移修复）：**P2-1**——删 `lib/core/extensions.dart`（229 行 14 个 extension 全库零引用，含拼写错误类名 `ContextExtentions`）、`lib/core/natural_compare.dart` + 连带 `test/core/natural_compare_test.dart`（生产零引用，测试数 1829→1823）；SnackBar 收敛：`official_anki_repair_center_page.dart` `_snack`、`anki_official_review_gate.dart` `_snackFailClosed` 改走 `TurnaSnackBar.show`（磨砂悬浮气泡，统一视觉与无障碍降级）。**P2-3**——真源路径漂移：CLAUDE.md（2 处）、`tool/gui/src/theme_tokens.py`（2 处 docstring/注释）、`tool/gui/src/theme.py`（1 处 docstring）的 `lib/views/theme.dart` → 实际位置 `lib/core/theme.dart`。`ambient_backdrop.dart` 雾团注释修正：`0x4FC3DC`/`0xE8A87C` 实为「湿地晨光」氛围专用冷青/暖杏，注释误称 brandSky/brandReed（真值 `0xFF4A95A8`/`0xFF78C7B8`）——核对设计意图后确认为氛围层有意离板色，修注释不换 token（视觉零变化）。`achievement_ui_catalog.dart` 7 处裸 `Color(0xFF...)`（系列强调色 + rarity 色，与 league token 刻意不同色值）与 `share_progress_card.dart` 5 处海报专用色（固定 light theme 自足渲染）分别提升为本文件命名常量（`_dailyFocusGold`/`_shareXpBoltAmber` 等），不自作主张并入 league/brand token。`dart analyze` lib+test **No issues found**；`dart format` 改动文件已合规；全量 `flutter test --exclude-tags golden` **1823 passed / 0 failed**。

Generated: 2026-09-13（代码治理 批2 / P1-2+P1-3：views DAO 收口 + 巨型 build 拆分）：**P1-2**——全库扫描发现 views 内 Official Anki DAO 直连点实为 6 文件 15 处（`official_pending_import_banner`/`anki_deck_stats_page`/`anki_official_review_gate`/`storage_diagnostics_page`/`anki_card_browser_page`/`official_anki_repair_center_page` 8 处）。新增 application 门面 `OfficialAnkiCatalogService`（`lib/application/anki_official/official_anki_catalog_service.dart`）：承接 catalog 文件存在性检查、source 列表/按 id 读取、pending import 取消、maintenance job 读写、integrity check、repair 快照等全部 view 侧 DAO 访问并 re-export `OfficialAnkiSourceRow`；review-gate 的「catalog 存在 + source 状态 → 引擎选路」规则收为服务命名方法；`OfficialAnkiSourceAwareStats`/`SourceAwareBrowser` 构造改收 catalog 访问而非强制 view 实例化 DAO；六页面全部改为消费门面。分层守卫测试新增 `views must not touch official-anki DAOs directly`（views 下 DAO 文件 import + `*Dao(` 构造式双规则，防回潮）。**P1-3**——`anki_review_screen`：`_AnkiDueHero.build`（~180 行）拆为 `_AnkiCaughtUpCard`/`_AnkiDueHeroHeader`/`_AnkiReviewAllButton` 三个私有 widget；牌组排序/置顶/卸载三处业务操作从 State 移入 `CourseProvider` 新方法 `reorderAnkiDecks`/`pinAnkiDeck`/`uninstallAnkiDeck`（排序语义不变：内置课第一、牌组按拖拽序紧随其后、非牌组课程保持相对序；卸载 false/异常=本次未删成语义与课程管理页对齐），State 仅留 UI 编排。`course_management_page`：`_CourseCard.build`（~200 行）拆为 `_CourseCardIcon`/`_CourseCardInfo`/`_CourseCardMenu`。`course_tree.dart`：计划所列 `_buildRevealSlot`/`_buildSectionScrollView` 已于 09-08 展开性能批次拆毕（现 39/53 行，计划行号基于旧快照），最大方法 `_buildLoadedSlivers` ~120 行，<200 验收已满足、不再重拆。`dart analyze`（改动文件+守卫）No issues found；`dart format` 改动文件零净变更；全量 `flutter test --exclude-tags golden` **1829 passed / 0 failed**（1828+1 新守卫用例）。

Generated: 2026-09-13（代码治理 批1 / P1-1：v2_view_rebuilder 类型漏洞 + strict-casts 开启）：`official_anki_v2_view_rebuilder.dart:113` `?? const []`（`List<dynamic>`，循环变量退化 dynamic、`:115`/`:121` 传参失编译期检查）改 `?? const <OfficialAnkiCardDescriptor>[]`；`analysis_options.yaml` `analyzer > language` 启用 `strict-casts: true`。开启后全库仅暴露 1 处（非预估 26——该漏洞为唯一动态源，修复后其余推断链自愈）：`official_anki_contract_integrity_test.dart:20` `(golden['payload'] as Map)['capabilities']` 补 `as Iterable<dynamic>` 显式断言。`dart analyze`（lib+test+tool 全库）**No issues found**；`dart format` 零变更；全量 `flutter test --exclude-tags golden` **1828 passed / 0 failed**。

Generated: 2026-09-12（多语言并存 × LibreLingo 兼容 P3 修复批）：**signatureChars 缺失回退**——`currentSpeechLanguages` 解析链尾部补 `?? ''`：导入包未声明 `signatureChars`（overlay null + registry 合成描述符 null）时显式传空串（检测器语义「无签名」），不再落到检测器的 null→土耳其 `ğĞıİşŞ` 默认（es-from-tr 类课程翻译侧含土耳其字符的文本不再被误判为目标语、用错语音）；内置 tr/fr manifest 恒声明集合不受影响，DI 失败兜底仍是有意的土耳其默认，检测器 null 契约不动（遗留调用方零变化）。**两处绕过 overlay 的显示**——`new_lesson_screen` AI 提问上下文与 splash `center_display` 改 overlay 优先（`displayNameOrNull`/`nativeLabelOrNull` ?? registry；splash displayName 直接复用 `LanguageProvider.displayName` getter），导入语言不再显示合成裸 code（"sw"）。**字典折叠泛化**——`lookupFoldKey` 的 `İ`→`i` 折叠改为全语言生效（plain `toLowerCase` 产出 `i`+U+0307 组合符是不可输入的 key），ASCII `I`→`ı` 无点规则从仅 tr 扩为 tr+az（`LanguageCodes.azerbaijani` 新常量）；索引/查询双侧同函数自动对称。**卸载媒体泄漏**——`uninstallBuiltinLanguage` 在 overlay `contains` 为 false 时先 `hydrate(db)` 重查（catalog 加载期 hydrate 静默失败不再导致 `imported_courses/<code>/media` 残留）。**小项**：删 `switchAwayFromImportedCode` 死代码（无调用方，已被 `CourseProvider.switchAwayIfActive` 取代，`course_catalog` import 一并移除）；`course-pack-format.md` 嵌套媒体描述改为与实现一致（`media/sub/x.jpg` 实际可用、按 `turnapack://<code>/<子路径>` 解析，仅 `media/media/` 双前缀拒绝）；转换器解析 LibreLingo `Audio:` 字段（真实课程音频哈希命名、仅经该字段引用，原按文件名=术语匹配大多接不上）——`parse_skill_words` 收集 `audio` 引用、`attach_word_media` 查找顺序 Audio 引用→Images→术语/slug。新增测试 dart +2（language_codes：İ 全语言折叠/az 无点规则）、python +1（Audio 字段接哈希文件+无字段术语匹配回归）；`flutter analyze` 改动 7 文件 **No issues found**；python discover **43 passed**；全量 `--exclude-tags golden` 实测 **1812 passed / 0 failed**。有意不修：`builtinLanguageOf` 空 map 回退 tr（已注释的查询失败降级）、全局跨语言状态（streak/成就/FSRS 权重，设计取舍）、遗留 prefs blob 迁移按当时 filter 打标（仅 pre-v25 单语时代数据）。

Generated: 2026-09-12（多语言并存 × LibreLingo 兼容 P1+P2 修复）：**P1 设置页语言选择器状态分裂**——`SettingsLanguageSelectorTile` 原来只写 `LanguageProvider`（TTS/显示名）不动 course scope：选课后课程树/SRS 过滤/错题/统计仍留旧语言，重启被 `_syncPracticeLanguage` 静默覆写回 `currentLanguage`，且列表只有 manifest 的 tr/fr 看不到导入语言。改为真切换器：数据源 `CourseProvider.catalogEntries`（isBuiltin 过滤，导入语言含 overlay displayName；catalog 空回退 manifest），选中即 `setScope(BuiltinCourseScope(code))`（镜像课程管理页 `_selectCourse` 序列：先即时 setLanguageCode 反馈、setScope 负责级联+持久化），失败 SnackBar。**P2 同 code 重导入残留污染**——`_clearLanguageTables` 只清内容表，换不同包后旧 id 的 `srs_states`/`review_events`/`mistakes` 行成幽灵（due 计数虚高、错题页回退显示裸 interactionId），lesson 进度集合与 `srs.lessonWordLinks` 永不清理。修法＝「id 差集孤儿清理 + 替换确认」（不做指纹全量清空——指纹无法区分换包与作者内容更新，会误毁正常更新的进度）：`CourseRepository` 新增 `resourceIdsForLanguage`/`languageHasContent`/`deleteOrphanedLearnerRows`（单事务删 srs_states/review_events 按 wordId/cardId not-in-live 集、mistakes 按 word/expression/grammarPoint dead 或 lessonId dead；mistake_aggregates 按日统计非资源键不动）；`LessonLinkStore.removeIds` 补上 doc 提到但不存在的删除 API（顺手修 replaceAll 幽灵注释）；importer 在 seed 前快照 old ids、seed 后按 live 集清理并自愈历史遗留孤儿，prefs 侧 `removeLessonIds`/`removeIds` + `reloadFromStorage`/`reloadFromPrefs` 显式刷缓存（`setLanguageFilter`/`setLanguage` 同语言早退，必须显式 reload，镜像 fun_lab_snapshot_service）；新构造参数 `confirmReplaceExisting`（DB 已有该语言内容行时回调，返回 false 抛新异常 `CoursePackImportCancelled` 静默中止；null=无门直接替换，reinstall 卸载后无内容行天然不触发）；课程管理页接「替换现有课程？」确认弹窗（复用 `_confirmDelete` 样式 + 取消 SnackBar，AppStrings +4）。取舍：两包共享词（同 code 同术语文本）SRS 记忆跨包延续视为合理语义（同语言同词=同一词汇项），仅死 id 清除。新增测试 4（importer：不同 id 集清孤儿保共享/同包重导入保进度+自愈遗留幽灵/拒绝确认零写入/接受确认正常导入）；`flutter analyze` 改动 8 文件 **No issues found**；全量 `--exclude-tags golden` 实测 **1810 passed / 0 failed**。

Generated: 2026-09-11（LibreLingo 导入缺陷修复批）：**转换器 id 碰撞修复**——`slugify` 折叠丢信息的词（重音对 más/mús、全部非拉丁词）自动加 `-<sha256[:6]>` 后缀，跨 skill 碰撞不再静默丢词/串错 wordId；sublesson/stage id 改由已消歧的 word id 派生；skill `Id` 撞名追加序号；模块 lesson 数按 30 切多 unit（修复 >40 skill 模块被 kMaxLessonsPerUnit 拒导入）。**题目质量**：MC/listenAndPick 选项确定性乱序（correctIndex 不再恒 0）、干扰项排除题干文本、reorder 干扰 chip 混入 shuffle（不再钉在尾部）、无/自译词条不再产照抄题。**方向性**：Mini-dictionary 按 `Language.Name`/`For speakers of.Name` 选目标语桶（-from-XX 课程不再拿反）、多义项逗号合并、裸词目跳过；`Special characters` 自动映射 `signatureChars`。**导入器**：`importFromFile` 改异步读、zip 限额按 `entry.content` 真实解压字节计（原为声明值）、index `sections[].id` 与 section 文件 id 一致性校验、嵌套 `media/` 路径拒绝、uninstall marker 移到 seed 成功后清（重导入失败语言保持可恢复）、解压媒体 staging 目录+原子交换（中途失败不留半写 `media/`）、非 UTF-8 包干净报错。**音频**：word 的 turnapack:// audioAsset 文件缺失时回退 TTS 词条、listen 内容缺文件回读 transcript（备份不含 `imported_courses/` 的已知缺口不再静默）。新增测试 python +9（42 passed）、dart importer +6；`flutter analyze` 改动文件 No issues；真实 ES 课程包（`../dist/es.turnapack`）经新转换器重建后 when-present 导入测试实跑通过。

Generated: 2026-09-11（`.turnapack` 收尾：真实课程验收 + 三态进度）：**转换器双形态修复**——真实仓库（`LibreLingo-ES-from-EN`，浅克隆于 `../LibreLingo-ES-from-EN`）的 `Modules`/`Skills`/`New words` 是头块平级节点且 module 名带尾斜杠，test-1 fixture 是嵌套形态；`convert_course` 三层（course/module/skill）统一 `{**doc, **header}` 合并兼容两者 + module 名 `strip("/")`。此前真实课程转换直接产出 0 sections（DoD §11「真实课程产出 validate 包」当时无证据）。**端到端证据**：真实课程转换+`course_cli.py validate` 通过 → `../dist/es.turnapack`（417KB v1）；新增 when-present 测试 python `test_real_es_course_converts_and_validates_when_present` + dart `real LibreLingo ES pack imports when present`（导入后 catalog 出现 `BuiltinCourseScope('es')`，本地均实跑通过，产物缺省自动 skip）。**导入器打磨**：`onPhase` 回调（decoding/validating/writing）接 UI 三态进度对话框（`ValueListenableBuilder`，l10n 三条文案此前已备未用）；解压媒体落盘移到 `seedLanguageFromSource` 成功之后——seed 失败不再删除/覆写旧安装的解压媒体（DB 零残留语义不变）。**清理**：删误产垃圾文件 `%TEMP%ll.json`（GitHub API 301 响应体）；导入错误路径补 `progressShown` 守卫——对话框打开前抛错（如「课程数据库尚未就绪」）时不再 `maybePop` 误弹管理页本身；`course-pack-format.md` 补 YAML 双形态说明、修仓库外计划文档引用。`flutter analyze` **No issues found**；python discover **33 passed**（+2：扁平布局回归、真实课程）；全量 `--exclude-tags golden` 实测 **1800 passed / 0 failed**（1799+1，上一条「估计 1799」已实测确认）。

Generated: 2026-09-11（`.turnapack` v2 包内媒体）：zip（`pack.json` + `media/`）+ `turnapack://<code>/file` 改写；转换器扫描 `images/`/`media/`/`audio/` 与 `--media-dir`，LibreLingo `Images` 进 `showWord.imageAsset`、同 stem 音频进 vocab/听力；导入 zip-slip 守卫、解压到 `imported_courses/<code>/media/`，卸载只删解压媒体保留 pack；`AudioController` / `CachedAssetImage` / `AnkiMediaStrip` / `ShowWord` 接线。定向 **12+4+19+N renderer**；python discover **31 passed**（+1 媒体 zip）。估计非 golden **1799**（1789+10：importer +3、media helper 4、audio 3）。

Generated: 2026-09-11 (`.turnapack` 运行时导入 A–D)：离线 `tool/librelingo_import.py` + `test/librelingo_import_test.py`（11）+ vendored LibreLingo `test-1` → `test/fixtures/mini.turnapack`；app `CoursePackImporter` + seeder `CourseSource`/`seedLanguageFromSource` 外层事务 + 跨语言 seen 集排除 `anki` + `importedLang:<code>` 叠加层（catalog 显示名 / TTS / signatureChars）；课程管理页「导入课程包」入口、license 署名、卸载后从持久化 pack 恢复。文档 `docs/authoring/course-pack-format.md` + [ADR 0043](../docs/decisions/0043-turnapack-external-courses.md)。定向 **13 passed**（importer 9 + course_management 4）；`python -m unittest discover -s test -p "*_test.py"` **30 passed**；`flutter analyze` 改动文件 **No issues found**。全量 `--exclude-tags golden` 未跑，估计 **1789**（1780+9）。

Generated: 2026-09-11 (多语言稳定性修复 P3：写路径与检测泛化)：**AI 写库语言标记**——`AiCourseProvider._writeSectionToDb`/`_writeResources` 全部 companion 补 `languageCode`（payload `language` 字段优先，否则 `LanguageContentStore.activeCode`；此前列默认 'tr'，非土耳其语写入静默归错语言），`updateLessonInDb` 两个 upsert 携带 `existing.languageCode`（复合 PK 下缺省会 fork 出 `(tr,id)` 孪生行、`lessonById` 的 `getSingleOrNull` 当场抛错）；新增 `_assertNoCrossLanguageIdCollision` 写前拒收他语言已占用的 section/unit/lesson id（运行时按裸 id 查询，schema 允许的跨语言同 id 在读侧是炸点）。**bulk 写入口参数化**：`bulkInsertCourseTree`/`bulkInsertVocabulary` 加 `languageCode`（null 保持原 Anki/'tr' 推导），`ICourseRepository` 接口与 `_CleanupRepo` fake 同步。**语言检测泛化**：`LanguageDescriptor.signatureChars` 新 manifest 字段（tr=`ğĞıİşŞ`、fr=拉丁变音符集），`LanguageDetector` 五方法加 `signatureChars` 形参（null=原土耳其默认，空串=显式无签名），`SpeechLanguages`/`currentSpeechLanguages`/三处调用点接线——法语 "écouter"/"français" 不再被误判 native 用英语音读。**section→语言归因精化**：`CourseCatalog.builtinLanguageOf`——map 非空但缺 id（v2 view 等非表行）不归任何内置语言；仅整表查询失败（map 空）才退化旧 tr 兜底，`_sectionInScope` 与 catalog 计数两处统一。**资源池 id 互斥**：`collectResourceIdCollisionErrors`（srs_states PK `(language_code, word_id)` 无 queue 列，跨池同 id 会互相覆盖排期行）进 `_seedLanguage` 写前检查 + `validateCourse` 新可选 `grammarPoints` 形参；`_seedLanguage` 重构为先解析三资源、校验、再落库（`_seedSections`/`_seedGrammarPoints`/`_seedExpressions` 改收解析后列表）。**小修**：`seedLanguage` 未知语言 guard 死代码改查 manifest 成员表（`byCode` 会合成描述符，原比较恒真）；`loadExpressions`/`loadGrammarPoints` 的 activeCode 比较补 canonicalize（'french' alias 不再漏 `publishGlobals`）；词典 term 键改 `LanguageCodes.lookupFoldKey`（土耳其 İ→i、I→ı 本地感知折叠，索引端 `_loadFresh` 与查询端 `getWordMeaning`/`searchDictionary` 双侧对称，translation 侧仍纯 lowercase）。新增测试 19：`ai_course_language_write_test` 4（payload 语言标记/activeCode 回退/跨语言 id 拒收/upsert 不 fork 孪生行）、`course_catalog_test` 4（builtinLanguageOf 三态 + ghost shell 不计入 tr）、seeder_fault_tolerance +2（未知语言返回 false、池内 id 互斥跳过）、detector signature 组 +4、`language_codes_test` 4、coexistence +1（alias 发布）。`flutter analyze` **No issues found**。全量 `--exclude-tags golden` **1780 passed / 0 failed**（1761+19）。

Generated: 2026-09-11 (删除 Playground 功能)：P0+P1 已交付但 P2 会话执行始终未接线（8 模式全为「即将推出」死格），经决策整体移除。删 `lib/application/playground/` 6 文件（eligibility/models/content_source/index/index_cache/assembler）、`lib/views/playground/language_playground_page.dart`、`LanguagePlaygroundRoute`（routing.gr.dart 经 build_runner 重生成）、Play Hub 顶部 Hero 块与资格 select、`play_tiles.dart` 的 `PlaygroundHero` 组件、~30 个 `playground*` l10n 键（保留混在同块的 `authoringMoved*`/`aiCardExplainUnsupported`）、`cache_diagnostics_registry` 的 `_PlaygroundCacheAdapter` 注册与类、`performance_trace` 的 `playground.index` 阈值、`CosmeticSurface.playground` 枚举值（无任何商品使用）与 `avatar_ring` primary-surface 分支、`CourseProvider.loadedSectionCount` 死 getter（唯一消费者在 playground_index）；`docs/language-playground-implementation-plan.md` 一并删除，project-guide §5.11/§15 引用同步清理。测试 −34 净（删 35：eligibility 4 + assembler/source 18 + index 3 + reentrancy 2 + 页面 widget 8；play_hub 10→9：3 个 playground 用例改写为 2 个 scope 队列用例；新增 `test/application/section_load_reentrancy_test.dart` 2 测，把原 reentrancy 回归中 provider 层的「先注册 in-flight future 再 notify」去重不变量保留为不依赖 playground 的版本）。`flutter analyze` **No issues found**。全量 `--exclude-tags golden`：**1761 passed / 0 failed**。play_hub light/dark golden 随 Hero 移除重生成通过。

Generated: 2026-09-09 (多语言稳定性修复 P2)：**schema v25→v26**——sections/units/lessons/lesson_contents 四张内容表复合主键 `(language_code, id)`/`(language_code, lesson_id)`（v25 同款四步重建迁移，单事务，`COALESCE` 回填 'tr'，`rebuildCompositePk` suffix 参数化）；同时删除三处装饰性 FK 子句（运行时从未 `PRAGMA foreign_keys = ON`，级联由 `deleteBuiltinLanguage` 事务手工完成；复合 PK 下单列引用不是合法父键）；`course_database.g.dart` 经 build_runner 重生成。Anki 导入树（languageCode='anki'）与内置课自此天然隔离，跨语言 DoUpdate 覆盖不可能。**seeder 容错**：`seedIfNeeded` 按语言 try/catch（一门损坏只跳过该语言，不再炸全部启动；`seedLanguage` 重灌路径保持抛错）+ `_seedSections` 的 seen 集合从其他**内置**语言 DB 行预填充（跨语言撞 id 给出明确 `CourseValidationException`；排除 anki 行避免导入 id 挡内置重灌）。**AI grounding 按语言取数**：`AiGroundedResourceProvider` 三处查询传当前 `LanguageProvider.selectedLanguageCode`（isRegistered 防护，null 不过滤保测试兼容）。**冷启动卸载 scope 兜底**：`_restoreScopeFromPrefs` 对 BuiltinCourseScope 查 uninstall marker，命中回退 `_fallbackBuiltin` 并持久化（修卸载中途被杀 → Learn 永久空树）。**课时进度卸载级联**：`LessonProgressProvider.removeLessonIds` + `CourseRepository.lessonIdsForLanguage`，`uninstallBuiltinLanguage` 删库前收集 id、删后清 prefs 集合与计数（成就保持单调解锁=历史事实，记录取舍）。**undo 事件删除语言化**：`deleteLatestForCard(cardId, {languageCode})`，undoReview 传快照语言。**备份自动恢复**：`restoreCourseDbFromBackup`（最新 `course.db.v*.bak` 覆盖损坏库+清 wal/shm），locator 开库后强制 `SELECT COUNT(*) FROM sqlite_master` 触发迁移，失败→close→恢复→重开一次→仍败 rethrow（修迁移失败=崩溃循环；取舍：无同版本备份时回退旧快照优于死循环）。新增测试 8：`seeder_fault_tolerance_test` 2（容错跳过/重灌仍抛）、`ai_grounded_resource_language_test` 2（按语言/无 DI 不过滤）、lesson_progress +2、coexistence +1（scope 兜底）、language_migration +3（v26 复合 PK 行保留+跨语言共存+table_info 旗标、备份恢复、无备份返回 null）；既有 v24 迁移断言改用 `kSchemaVersion` 常量。`flutter analyze` **No issues found**。全量 `--exclude-tags golden` **1769 passed / 0 failed**（1759 基线 + 本轮 10）。

Generated: 2026-09-09 (多语言切换竞态修复 P0+P1)：`SrsQueueProvider` 复习/undo 语言快照——`_doReviewItem`/`undoReview` 入口快照 `_languageFilter`，`countFailsOnLocalDay`/`upsert`/`insertEvent` 全用快照值，await 后 filter 已变则跳过 `_commit`（旧语言 map 不再塞回新语言缓存）但 DB 照常按快照语言持久化（修高危：切换窗口内旧语言卡片写进新语言行 + 新语言内存缓存被整段清掉）；`ensureLoaded` 加 `_loadGeneration` 守卫（连切两次语言，被放弃的慢加载不再 putIfAbsent 混入混合缓存）；`setLanguageFilter` 的 `_bumpReviewDataRevision` 从水合前移到水合后（空快照不再按新 revision 驻留 dashboard/insights 缓存）；`_writeBatch`/`persist`/`_commitAndPersist`/`_migrateFromPrefs` 语言参数显式传递。`clearQueue` 加可选 languageCode，`clear()` 只清当前语言（内容更新重置不再跨语言清空，与错题本 clear 语义对齐）。`_syncPracticeLanguage`：`activate` 失败改 return（下游 5 provider 不再照切成混合语言态）；`setLanguageCode` 后 `unawaited(cacheLanguage())`（Home 重进不再被旧 prefs 拉回）。main.dart SRS 预热改 `setLanguageFilter(persisted scope 语言 ?? default)`（非默认语言用户冷启 due 徽标不再闪错语言数据）。`PostRestoreReloadRegistry`：srs/grammarReview 步骤先按恢复后的语言 `setLanguageFilter` 再 `reloadFromStorage`；mistakes 步骤 `reloadFromPrefs` 后真重载（修恢复后错题本滞空）；删除死步骤 srsQueue（`isRegistered<SrsQueueProvider>` 恒 false）。新增测试 8：`srs_language_switch_race_test` 5（复习/undo 竞态、连切 generation、clear 分语言、revision 每次 bump 恰一次）、`post_restore_reload_registry_test` 2（恢复后队列重指向+错题真重载、同语言恢复仍刷新）、srs_state_dao_test +1（clearQueue 分语言）。`flutter analyze` **No issues found**。全量 `--exclude-tags golden` **1759 passed / 0 failed**（1749 基线 + firebase 2 + 本轮 8）。

Generated: 2026-09-08 (Firebase FlutterFire Android)：`firebase_core` + `lib/firebase_options.dart`（turna-d0d5e / `me.dsdogs.turna`）+ `main` 仅 Android `Firebase.initializeApp`。定向 **2 passed**（`test/firebase_options_test.dart`）。

Generated: 2026-09-08 (多语言改造收尾修复 P0–P2)：卸载持久化——`uninstalled:<code>` meta 标记（seeder 跳过 + catalog 排除 + 课程管理页「已卸载的语言」恢复区 + `reinstallBuiltinLanguage` 重灌，tr 卸载补删 legacy contentVersion 键）；v25 迁移包 `db.transaction` + 迁移前 `VACUUM INTO course.db.v<v>.bak` 备份（新 `course_db_backup.dart`，locator 开库前调用，失败告警不阻塞）；`recentReviews` 带 languageCode（导师上下文不再混语言，来源 MistakeProvider.languageCode；clearQueue/Fun Lab 经核实为全局语义不动）；AI/诊断/文案残余归零——unified_review/ai_tutor_chat/dictionary/ai_diagnosis/ai_lesson_helper/textbook_import/system_health/ai_hint_chat/assembler 默认改当前课程语言（`LanguageProvider` + isRegistered 防护），l10n 带参化（splashLearnLanguage/voiceMissing×3 + manifest 增 `nativeLabel` 字段）+ `TtsPreferredStatus.turkishVoiceMissing→voiceMissing` + 通知文案调度时快照语言；`TargetLanguage` 枚举删除（enums.dart 整文件 + selectedLanguage/setLanguage/getEnumValue/死方法 sectionInActiveScope）；预热改 scope 语言优先（main.dart 读 persisted scope 再 load 三件套）；`StudyLogRepository.deleteByLanguage`（logs+recent+dailyStats 前缀清理，tr 含无前缀旧键）接入卸载；store 加固（`_inFlight` whenComplete 清理、activate 成功后才切 active）；mistakes 迁移幂等（损坏 JSON 返回 false 保留 prefs 重试 + provider try/catch）；`_syncPracticeLanguage` 各 catch 加日志；迁移测试补 `contentVersion:tr` 分桶断言。新增测试 5（coexistence +2：卸载持久化/恢复、recentReviews 隔离；migration +1：备份；study_log +2：deleteByLanguage）。门禁 coexistence/migration/no-turkish-assets 全绿。`dart analyze lib test` **No issues found**。全量 `--exclude-tags golden` **1749 passed / 0 failed**（1744+5）。grep 审计：lib 下 'Turkish' 仅剩 manifest packedFallback 数据声明；'tr' 硬编码清零（LanguageCodes 常量/迁移 DEFAULT/资产路径/注释除外）。

Generated: 2026-09-08 (Insights 语言隔离)：`ReviewProgressProvider._historyFilter` / `listSources` 带 `SrsProvider.languageFilter`；切语言 bump `ReviewDataRevision`。新增 2 测（coexistence Insights + progress snapshot）。门禁 coexistence/migration/no-turkish-assets **8 passed**。`dart analyze lib test` **No issues found**。全量 `--exclude-tags golden` **1744 passed / 0 failed**。

Generated: 2026-09-08 (多语言并存收口)：成就投影 `getIt` 走 `di/injection.dart`；dashboard/history/stats/FSRS/memory-curve/achievements 查询带 languageCode；隔离测试打 shipped DAO/provider API；`countFailsOnLocalDay` 测试桩签名对齐。定向 coexistence/migration/no-turkish-assets **7 passed**。`dart analyze lib test` **No issues found**。全量 `flutter test --exclude-tags golden` **1742 passed / 0 failed**（日志 `{SCRATCH}/flutter_test.log`）。golden 未跑。

Generated: 2026-09-08 (多语言并存)：schema v24→v25，内容/SRS/历史/错题加 languageCode；manifest 驱动 Turkish+French fixture 同库并存、目录分条、练习队列隔离、卸载级联、v24 升级回填 tr。新增测试 7（coexistence 5 + migration 1 + no-turkish-assets 1）。定向 coexistence/migration/no-turkish-assets **7 passed**；seeder/schema/course_provider/mistake 定向全绿。`dart analyze lib test` No issues found。全量 `--exclude-tags golden` **1741 passed / 1 failed**（course_management 仍找枚举「Turkish」），随后改为 catalog `displayName`，该文件 **4/4**；非 golden 预期 **1742 passed / 0 failed**（1735+7）。golden 本机既有差异仍在册。
