...
Generated: 2026-08-22 (首页底栏 hide-on-scroll：`ScrollHidePolicy` 纯函数 + `ScrollHideBar`（AnimatedSlide 1.15 / IgnorePointer），`HomePage` 对 IndexedStack 听 `UserScrollNotification`，切 tab 强制显示，不改 overlayExtent。新增 9 测：policy 6 + bar widget 3。`flutter analyze` 改动文件 0 issue。定向 `flutter test` **14 passed**。全量估计 **1308** = 1299+9。)
