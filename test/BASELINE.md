...
- Python:
  ...
  - `tool/gui` suite (from repo root: `python3 -m unittest discover -s tool/gui/tests -p "test_*.py"`) — **377 passed** (P3 新增 `test_json_editor.py` 5、`test_diff_view.py` 4、`test_ai_generator_dialog.py` B1 回归 3)。全量 GUI 套件 offscreen 本机可跑（3.9s）。
