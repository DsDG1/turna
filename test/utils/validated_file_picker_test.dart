...
void main() {
  group('ValidatedFilePicker suffix normalization and matching', () {
    test('normalizes extensions to lowercase with leading dot', () {
      final suffixes =
          ValidatedFilePicker.normalizedSuffixes(['apkg', '.COLPKG', 'MD']);
    ...
    test('hasAllowedSuffix matches case-insensitively', () {
      ...
      expect(
          ValidatedFilePicker.hasAllowedSuffix('/storage/vocab.apkg', suffixes),
          isTrue);
      expect(
          ValidatedFilePicker.hasAllowedSuffix('/storage/vocab.APKG', suffixes),
          isTrue);
      expect(
          ValidatedFilePicker.hasAllowedSuffix(
              '/storage/vocab.colpkg', suffixes),
          isTrue);
      expect(
          ValidatedFilePicker.hasAllowedSuffix(
              '/storage/vocab.apkg ', suffixes),
          isTrue);
      expect(
          ValidatedFilePicker.hasAllowedSuffix('/storage/vocab.txt', suffixes),
          isFalse);
    });

    test('findMatchingSuffix identifies correct suffix', () {
      final suffixes = {'.apkg', '.colpkg'};
      expect(ValidatedFilePicker.findMatchingSuffix('deck.apkg', suffixes),
          equals('.apkg'));
      expect(ValidatedFilePicker.findMatchingSuffix('deck.COLPKG', suffixes),
          equals('.colpkg'));
      expect(
          ValidatedFilePicker.findMatchingSuffix('deck.txt', suffixes), isNull);
    });
    ...
    test('findWrappedSuffix identifies WeChat and browser wrapped extensions',
      ...
      final suffixes = {'.apkg', '.colpkg'};
      // WeChat download (.1, .2)
      expect(ValidatedFilePicker.findWrappedSuffix('deck.apkg.1', suffixes),
          equals('.apkg'));
      expect(
          ValidatedFilePicker.findWrappedSuffix(
              'collection.colpkg.1', suffixes),
          equals('.colpkg'));
      // Browser download (.bin, .zip, .download)
      expect(ValidatedFilePicker.findWrappedSuffix('deck.apkg.bin', suffixes),
          equals('.apkg'));
      expect(ValidatedFilePicker.findWrappedSuffix('deck.apkg.zip', suffixes),
          equals('.apkg'));
      expect(
          ValidatedFilePicker.findWrappedSuffix('deck.apkg.download', suffixes),
          equals('.apkg'));
      // Download counter
      expect(ValidatedFilePicker.findWrappedSuffix('deck.apkg(1)', suffixes),
          equals('.apkg'));
      expect(ValidatedFilePicker.findWrappedSuffix('deck.apkg_1', suffixes),
          equals('.apkg'));
      // Direct extension
      expect(ValidatedFilePicker.findWrappedSuffix('deck.apkg', suffixes),
          equals('.apkg'));
      // Unrelated extensions should not match
      expect(
          ValidatedFilePicker.findWrappedSuffix('photo.jpg', suffixes), isNull);
      expect(
          ValidatedFilePicker.findWrappedSuffix('notes.txt', suffixes), isNull);
    });
  });
  ...
  group('ValidatedFilePicker.resolvePickedFilePath', () {
    ...
    test('returns direct path when rawPath has allowed suffix', () async {
      final file = File('${tempDir.path}/deck.apkg')
    ...
    test('stages file when path has no extension but name has allowed suffix',
        () async {
      // Common Android SAF behavior: cached as hash/number
      final file = File('${tempDir.path}/1725638491')
    ...
    test('stages WeChat .apkg.1 file to clean .apkg path', () async {
      final file = File('${tempDir.path}/turkish.apkg.1')
        ..writeAsStringSync('wechat_deck');
      final platformFile = PlatformFile(
        name: 'turkish.apkg.1',
        size: 11,
        path: file.path,
    ...
    test('stages browser .apkg.bin file to clean .apkg path', () async {
      final file = File('${tempDir.path}/download.apkg.bin')
        ..writeAsStringSync('browser_deck');
      final platformFile = PlatformFile(
    ...
    test('detects real Anki package even when named .bin or .zip', () async {
      ...
      final fixture =
          File('test/fixtures/anki_official/packages/01-basic-unicode.apkg');
