...
void main() {
  ...
  group('detect - non-Latin scripts', () {
    test('Chinese (Han) -> zh', () {
      expect(detector.detect('你好', targetLanguage: target, nativeLanguage: native), 'zh');
      expect(detector.detect('谢谢', targetLanguage: target, nativeLanguage: native), 'zh');
    });

    test('Japanese (Kana) -> ja even with Kanji', () {
      expect(detector.detect('こんにちは', targetLanguage: target, nativeLanguage: native), 'ja');
      expect(detector.detect('おはよう', targetLanguage: target, nativeLanguage: native), 'ja');
    });

    test('Korean (Hangul) -> ko', () {
      expect(detector.detect('안녕하세요', targetLanguage: target, nativeLanguage: native), 'ko');
    });

    test('Cyrillic -> ru', () {
      expect(detector.detect('привет', targetLanguage: target, nativeLanguage: native), 'ru');
      expect(detector.detect('спасибо', targetLanguage: target, nativeLanguage: native), 'ru');
    });

    test('Arabic -> ar', () {
      expect(detector.detect('مرحبا', targetLanguage: target, nativeLanguage: native), 'ar');
    });

    test('Thai -> th', () {
      expect(detector.detect('สวัสดี', targetLanguage: target, nativeLanguage: native), 'th');
    });

    test('Devanagari -> hi', () {
      expect(detector.detect('नमस्ते', targetLanguage: target, nativeLanguage: native), 'hi');
    });

    test('Greek -> el', () {
      expect(detector.detect('γεια', targetLanguage: target, nativeLanguage: native), 'el');
    });

    test('Hebrew -> he', () {
      expect(detector.detect('שלום', targetLanguage: target, nativeLanguage: native), 'he');
    });
  });

  group('detect - Latin script', () {
    test('Turkish-specific letters -> target language', () {
      expect(detector.detect('teşekkürler', targetLanguage: target, nativeLanguage: native), target);
      // 'günaydın' has dotless ı (U+0131) -> Turkish-specific.
      expect(detector.detect('günaydın', targetLanguage: target, nativeLanguage: native), target);
      // ı (dotless i) and İ (dotted I) are Turkish-specific.
      expect(detector.detect('hayır', targetLanguage: target, nativeLanguage: native), target);
      expect(detector.detect('İstanbul', targetLanguage: target, nativeLanguage: native), target);
    });

    test('plain ASCII Latin (incl. "Merhaba" with no diacritics) -> native', () {
      expect(detector.detect('hello', targetLanguage: target, nativeLanguage: native), native);
      expect(detector.detect('Merhaba', targetLanguage: target, nativeLanguage: native), native);
      expect(detector.detect('merhaba', targetLanguage: target, nativeLanguage: native), native);
    });
    ...
    test('plain ASCII Latin respects a different native language', () {
      ...
      // native language.
      expect(detector.detect('hello', targetLanguage: target, nativeLanguage: 'zh'), 'zh');
    });

    test('ç ö ü alone are NOT Turkish-specific (shared with French/German)', () {
      // "café" has é/ç-like Latin but no ğ/ı/ş -> native fallback.
      expect(detector.detect('café', targetLanguage: target, nativeLanguage: native), native);
      expect(detector.detect('über', targetLanguage: target, nativeLanguage: native), native);
    });
  });

  group('detect - edge cases', () {
    test('empty -> target', () {
      expect(detector.detect('', targetLanguage: target, nativeLanguage: native), target);
      expect(detector.detect('   ', targetLanguage: target, nativeLanguage: native), target);
    });

    test('digits / punctuation only -> target', () {
      expect(detector.detect('123', targetLanguage: target, nativeLanguage: native), target);
      expect(detector.detect('!!!', targetLanguage: target, nativeLanguage: native), target);
    });
  });
  ...
  group('inferOptionLanguage (MCQ direction inference)', () {
    ...
    test('plain-Latin Turkish prompt (no diacritics) defaults options to target',
        () {
    ...
    test('reliable-signal option overrides the inferred language', () {
      ...
      expect(
        detector.detectOption('teşekkürler',
            optionLanguage: target, targetLanguage: target, nativeLanguage: native),
        target,
      );
    ...
    test('plain-Latin option follows the inferred option language', () {
      ...
      expect(
        detector.detectOption('thank you',
            optionLanguage: native, targetLanguage: target, nativeLanguage: native),
        native,
      );
    });
  });
  ...
  group('detectCardPair (Anki front/back)', () {
    test('front Turkish-specific, back plain Latin -> front target, back native', () {
    ...
    test('back Turkish-specific, front plain Latin -> back target, front native', () {
    ...
    test('both faces plain-Latin Turkish (no diacritics) -> independent -> native', () {
    ...
    test('front Turkish-specific, back Turkish-specific -> both independent (target)', () {
